...
def _flow_components(load, available_renewable, charge, discharge, export_limit):
    """唯一Q3/Q4来源分解；支持标量或numpy广播数组。"""
    load = np.asarray(load, dtype=float)
    renewable = np.asarray(available_renewable, dtype=float)
    charge = np.asarray(charge, dtype=float)
    discharge = np.asarray(discharge, dtype=float)
    export_limit = np.asarray(export_limit, dtype=float)
    direct = np.minimum(load, renewable)
    renewable_surplus = np.maximum(renewable - direct, 0.0)
    renewable_charge = np.minimum(charge, renewable_surplus)
    grid_charge = charge - renewable_charge
    renewable_after_charge = renewable_surplus - renewable_charge
    deficit = np.maximum(load - direct, 0.0)
    storage_to_load = np.minimum(discharge, deficit)
    grid_for_load = deficit - storage_to_load
    storage_sell = discharge - storage_to_load
    renewable_sell = np.minimum(renewable_after_charge, np.maximum(export_limit, 0.0))
    remaining_export_capacity = np.maximum(export_limit - renewable_sell, 0.0)
    valid_no_discharge_dump = storage_sell <= remaining_export_capacity + TOL
    grid_purchase = grid_for_load + grid_charge
    grid_sell = renewable_sell + storage_sell
    curtailment = renewable_after_charge - renewable_sell
    net_grid = grid_purchase - grid_sell
    return {
        ...
        "StorageToLoad_MW": storage_to_load,
        "RenewableSell_MW": renewable_sell,
        "StorageSell_MW": storage_sell,
        "GridPurchaseForLoad_MW": grid_for_load,
        "GridPurchase_MW": grid_purchase,
        "GridSell_MW": grid_sell,
        "RenewableCurtailment_MW": curtailment,
        "NetGrid_MW": net_grid,
        "valid_no_discharge_dump": valid_no_discharge_dump,
    }


def attach_energy_source_columns(flow):
    x = flow.copy()
    comp = _flow_components(
        x["FacilityLoad_MW"].to_numpy(float),
        x["AvailableRenewable_MW"].to_numpy(float),
        x["Charge_MW"].to_numpy(float),
        x["Discharge_MW"].to_numpy(float),
        x["GridExportLimit_MW"].to_numpy(float),
    )
    if not np.asarray(comp.pop("valid_no_discharge_dump"), dtype=bool).all():
        raise RuntimeError("CON-Q3-8: 储能放电超过负荷与剩余外送容量，等价于放电弃置")
    for column, values in comp.items():
        x[column] = values
    # 兼容历史列名，但所有正式计算只读取来源显式列。
    x["RenewableUse_MW"] = x["RenewableDirectUse_MW"]
    x["Curtailment_MW"] = x["RenewableCurtailment_MW"]
    return x


def energy_source_audit(flow, scope):
    x = attach_energy_source_columns(flow)
    rows = []
    for region, g in x.groupby("Region", sort=True):
        energy_residual = (
            g["AvailableRenewable_MW"] + g["GridPurchase_MW"] + g["Discharge_MW"]
            - g["FacilityLoad_MW"] - g["Charge_MW"] - g["GridSell_MW"]
            - g["RenewableCurtailment_MW"]
        )
        source_residual = pd.concat([
            g["Charge_MW"] - g["RenewableCharge_MW"] - g["GridCharge_MW"],
            g["Discharge_MW"] - g["StorageToLoad_MW"] - g["StorageSell_MW"],
            g["GridPurchase_MW"] - g["GridPurchaseForLoad_MW"] - g["GridCharge_MW"],
            g["GridSell_MW"] - g["RenewableSell_MW"] - g["StorageSell_MW"],
            g["AvailableRenewable_MW"] - g["RenewableDirectUse_MW"]
            - g["RenewableCharge_MW"] - g["RenewableSell_MW"]
            - g["RenewableCurtailment_MW"],
        ], axis=0)
        denominator = float(g["AvailableRenewable_MW"].sum())
        if denominator <= 0:
            raise RuntimeError(f"CON-Q3-7: {scope}/{region} 新能源分母非正")
        rows.append({
            "scope": scope, "Region": region,
            "renewable_direct_use_MWh": float(g["RenewableDirectUse_MW"].sum()),
            "renewable_charge_MWh": float(g["RenewableCharge_MW"].sum()),
            "grid_charge_MWh": float(g["GridCharge_MW"].sum()),
            "renewable_sell_MWh": float(g["RenewableSell_MW"].sum()),
            "storage_sell_MWh": float(g["StorageSell_MW"].sum()),
            "renewable_curtailment_MWh": float(g["RenewableCurtailment_MW"].sum()),
            "renewable_utilization": float((g["RenewableDirectUse_MW"] + g["RenewableCharge_MW"] + g["RenewableSell_MW"]).sum() / denominator),
            "max_energy_balance_abs_residual_MW": float(energy_residual.abs().max()),
            "max_source_decomposition_abs_residual_MW": float(source_residual.abs().max()),
        })
    return pd.DataFrame(rows)


def equivalent_cycles(d, flow):
    storage = d["storage"].set_index("Region")
    rows = []
    for region in REGIONS:
        g = flow.loc[flow["Region"].eq(region)]
        s = storage.loc[region]
        usable = storage_soc_max(s) - float(s["MinSOC_MWh"])
        if usable <= 0:
            raise RuntimeError(f"CON-Q3-9: {region} SOC_max必须大于SOC_min")
        throughput = float((
            float(s["ChargeEfficiency"]) * g["Charge_MW"]
            + g["Discharge_MW"] / float(s["DischargeEfficiency"])
        ).sum())
        rows.append({
            "Region": region,
            "soc_min_MWh": float(s["MinSOC_MWh"]),
            "soc_max_MWh": storage_soc_max(s),
            "usable_soc_interval_MWh": usable,
            "internal_throughput_MWh": throughput,
            "equivalent_full_cycles": throughput / (2.0 * usable),
        })
    frame = pd.DataFrame(rows)
    return frame, float(frame["equivalent_full_cycles"].sum())


def _ramp_diagnostics(x):
    def total(column):
        return float(x.groupby("Region")[column].apply(
            lambda values: np.abs(np.diff(values.to_numpy(float))).sum()
        ).sum())
    return {
        "grid_purchase_ramp_MW": total("GridPurchase_MW"),
        "grid_sell_ramp_MW": total("GridSell_MW"),
        "net_grid_ramp_MW": total("NetGrid_MW"),
    }
...
def q3_baseline_flow(d):
    ...
    rt["Charge_MW"] = 0.0
    rt["Discharge_MW"] = 0.0
    rt["SOC_MWh"] = [float(storage.loc[r, "InitialSOC_MWh"]) for r in rt["Region"]]
    rt["GridExportLimit_MW"] = [grid_export_limit(row, storage.loc[row["Region"]]) for _, row in rt.iterrows()]
    ...
    return attach_energy_source_columns(rt)
...
def metrics_energy(x):
    x = attach_energy_source_columns(x)
    cost = np.sum(x["GridPurchase_MW"] * x["Price"] - x["GridSell_MW"] * x["SellPrice"])
    carbon = np.sum(x["GridPurchase_MW"] * x["CarbonIntensity"])
    available = float(x["AvailableRenewable_MW"].sum())
    if available <= 0:
        raise RuntimeError("CON-Q3-7: actual=0, threshold=positive renewable denominator")
    used = float((x["RenewableDirectUse_MW"] + x["RenewableCharge_MW"] + x["RenewableSell_MW"]).sum())
    peak = float(x.assign(_positive_net=x["NetGrid_MW"].clip(lower=0)).groupby("Region")["_positive_net"].max().sum())
    ramp = _ramp_diagnostics(x)["net_grid_ramp_MW"]
    return finite(cost, "cost"), finite(carbon, "carbon"), finite(used / available, "renewable"), finite(peak, "peak"), finite(ramp, "ramp")

def audit_storage_policy(d, flow, constraint_prefix):
    required = {
        "RenewableDirectUse_MW", "RenewableCharge_MW", "GridCharge_MW",
        "StorageToLoad_MW", "RenewableSell_MW", "StorageSell_MW",
        "GridPurchaseForLoad_MW", "RenewableCurtailment_MW", "NetGrid_MW",
    }
    if not required.issubset(flow.columns):
        raise RuntimeError(f"{constraint_prefix}: 缺少来源显式列 {sorted(required.difference(flow.columns))}")
    storage = d["storage"].set_index("Region")
    rows = []
    for r in REGIONS:
        x = flow.loc[flow["Region"].eq(r)].sort_values("Hour")
        s = storage.loc[r]
        previous = float(s["InitialSOC_MWh"])
        maxima = {"energy": 0.0, "soc": 0.0, "bound": 0.0, "mutex": 0.0, "source": 0.0, "net": 0.0}
        for row in x.itertuples():
            energy_residual = (float(row.AvailableRenewable_MW) + float(row.GridPurchase_MW) + float(row.Discharge_MW) - float(row.FacilityLoad_MW) - float(row.Charge_MW) - float(row.GridSell_MW) - float(row.RenewableCurtailment_MW))
            expected_soc = previous + float(s["ChargeEfficiency"]) * float(row.Charge_MW) - float(row.Discharge_MW) / float(s["DischargeEfficiency"])
            source_residuals = [
                float(row.Charge_MW) - float(row.RenewableCharge_MW) - float(row.GridCharge_MW),
                float(row.Discharge_MW) - float(row.StorageToLoad_MW) - float(row.StorageSell_MW),
                float(row.GridPurchase_MW) - float(row.GridPurchaseForLoad_MW) - float(row.GridCharge_MW),
                float(row.GridSell_MW) - float(row.RenewableSell_MW) - float(row.StorageSell_MW),
                float(row.AvailableRenewable_MW) - float(row.RenewableDirectUse_MW) - float(row.RenewableCharge_MW) - float(row.RenewableSell_MW) - float(row.RenewableCurtailment_MW),
            ]
            maxima["energy"] = max(maxima["energy"], abs(energy_residual))
            maxima["soc"] = max(maxima["soc"], abs(float(row.SOC_MWh) - expected_soc))
            maxima["bound"] = max(maxima["bound"], float(s["MinSOC_MWh"]) - float(row.SOC_MWh), float(row.SOC_MWh) - storage_soc_max(s), float(row.Charge_MW) - float(s["MaxChargePower_MW"]), float(row.Discharge_MW) - float(s["MaxDischargePower_MW"]), float(row.GridSell_MW) - float(row.GridExportLimit_MW), float(row.GridPurchase_MW) - float(s["MaxGridImport_MW"]), -float(row.GridPurchase_MW), -float(row.GridSell_MW))
            maxima["mutex"] = max(maxima["mutex"], min(float(row.Charge_MW), float(row.Discharge_MW)), min(float(row.GridPurchase_MW), float(row.GridSell_MW)))
            maxima["source"] = max(maxima["source"], *(abs(v) for v in source_residuals), max(0.0, float(row.StorageSell_MW) - max(0.0, float(row.GridExportLimit_MW) - float(row.RenewableSell_MW))))
            maxima["net"] = max(maxima["net"], abs(float(row.NetGrid_MW) - float(row.GridPurchase_MW) + float(row.GridSell_MW)))
            previous = float(row.SOC_MWh)
        terminal_residual = abs(previous - float(s["InitialSOC_MWh"]))
        rows.append({"Region": r, "max_energy_abs_residual_MW": maxima["energy"], "max_soc_abs_residual_MWh": maxima["soc"], "max_storage_bound_residual": maxima["bound"], "max_mutex_residual_MW": maxima["mutex"], "max_source_decomposition_abs_residual_MW": maxima["source"], "max_net_grid_identity_abs_residual_MW": maxima["net"], "terminal_soc_residual_MWh": terminal_residual})
    audit = pd.DataFrame(rows)
    max_residual = float(np.maximum(audit.drop(columns="Region").to_numpy(float), 0.0).max())
    if max_residual > TOL:
        fail(constraint_prefix, max_residual, TOL, "储能、来源分解或净电网审计失败")
    return audit
...
Q4_SCENARIO_FLOWS = {}
Q4_CARBON_CANDIDATE_BANK_ROWS = []
Q4_CARBON_NESTED_AUDIT_ROWS = []
...
def _q2_context(d, schedule):
    ...
    if "SearchSignal_CNY_per_MWh" in rt.columns:
        arrays["ElectricityPrice_CNY_per_MWh"] = np.vstack([
            rt.loc[rt["Region"].eq(r), "SearchSignal_CNY_per_MWh"].to_numpy(float)
            for r in REGIONS
        ])
    gpu = d["gpu"].set_index("Region")
...
def _no_action_policy(d, facility_override=None, **scenario):
    ...
    storage = d["storage"].set_index("Region")
    rt["AvailableRenewable_MW"] = rt["ScenarioRenewable_MW"]
    rt["Charge_MW"] = 0.0
    rt["Discharge_MW"] = 0.0
    rt["SOC_MWh"] = [float(storage.loc[r, "InitialSOC_MWh"]) for r in rt["Region"]]
    rt["GridExportLimit_MW"] = [grid_export_limit(row, storage.loc[row["Region"]]) for _, row in rt.iterrows()]
    rt["Price"] = rt["ScenarioPrice"]
    rt["SellPrice"] = rt["ScenarioSellPrice"]
    rt["CarbonIntensity"] = rt["CarbonIntensity_tCO2_per_MWh"]
    return attach_energy_source_columns(rt)

def ramp_min_edge_dp(net_by_time, feasible_by_time, initial_index, terminal_index=None):
    """相邻SOC边状态DP；返回小时末SOC索引路径与精确净电网总变差。"""
    nets=[np.asarray(v,float) for v in net_by_time]; feasible=[np.asarray(v,bool) for v in feasible_by_time]
    if not nets or len(nets)!=len(feasible): raise ValueError("ramp_min DP需要等长非空时域")
    n=nets[0].shape[0]
    if any(v.shape!=(n,n) for v in nets+feasible): raise ValueError("ramp_min DP矩阵必须为n*n")
    edge_dp=np.full((n,n),np.inf); edge_dp[initial_index,:]=np.where(feasible[0][initial_index,:],0.0,np.inf)
    predecessors=np.full((len(nets),n,n),-1,dtype=np.int16)
    for t in range(1,len(nets)):
        new_dp=np.full((n,n),np.inf); pred=np.full((n,n),-1,dtype=np.int16)
        for current in range(n):
            values=edge_dp[:,current][:,None]+np.abs(nets[t][current,:][None,:]-nets[t-1][:,current][:,None])
            values[:,~feasible[t][current,:]]=np.inf
            best=np.argmin(values,axis=0); new_dp[current,:]=values[best,np.arange(n)]; pred[current,:]=best
        edge_dp=new_dp; predecessors[t]=pred
        if not np.isfinite(edge_dp).any(): raise RuntimeError("ramp_min相邻SOC对DP无可行状态")
    if terminal_index is None: current_before,current_after=np.unravel_index(np.argmin(edge_dp),edge_dp.shape)
    else: current_after=int(terminal_index); current_before=int(np.argmin(edge_dp[:,current_after]))
    optimum=float(edge_dp[current_before,current_after])
    if not np.isfinite(optimum): raise RuntimeError("ramp_min终端状态不可达")
    path=np.empty(len(nets),dtype=np.int16)
    if len(nets)==1: path[0]=current_after
    else:
        for t in range(len(nets)-1,0,-1):
            path[t]=current_after; path[t-1]=current_before
            prior=int(predecessors[t,current_before,current_after]); current_after,current_before=current_before,prior
        if current_before!=int(initial_index): raise RuntimeError("ramp_min回溯未回到初始SOC")
    return path,optimum
...
def _storage_dp_policy(d, objective, facility_override=None, state_count=21,
    ...
    for region_no, region in enumerate(REGIONS):
        ...
        s = storage.loc[region]
        minimum, maximum, initial = float(s["MinSOC_MWh"]), storage_soc_max(s), float(s["InitialSOC_MWh"])
        if maximum <= minimum:
            raise RuntimeError(f"CON-Q3-9: {region} SOC_max必须大于SOC_min")
        ...
        n = len(states)
        before, after = states[:, None], states[None, :]
        ...
        transition_ok = (charge <= float(s["MaxChargePower_MW"]) + TOL) & (discharge <= float(s["MaxDischargePower_MW"]) + TOL)
        export_limits = np.asarray([grid_export_limit(row, s) for _, row in x.iterrows()], dtype=float)

        if objective == "ramp_min":
            net_by_time, feasible_by_time = [], []
            for t, row in enumerate(x.itertuples()):
                if t % 24 == 0: check_deadline(f"storage.{region}.ramp_pair", t)
                comp = _flow_components(float(row.FacilityLoad_MW), float(row.ScenarioRenewable_MW), charge, discharge, export_limits[t])
                net_by_time.append(np.asarray(comp["NetGrid_MW"], float))
                feasible_by_time.append(transition_ok & np.asarray(comp["valid_no_discharge_dump"], bool) & (np.asarray(comp["GridPurchase_MW"], float) <= float(s["MaxGridImport_MW"]) + TOL))
            path, _ = ramp_min_edge_dp(
                net_by_time, feasible_by_time, initial_index,
                initial_index if terminal_mode == "initial" else None,
            )
        else:
            dp = np.full(n, np.inf); dp[initial_index] = 0.0
            predecessors = np.full((len(x), n), -1, dtype=np.int16)
            for t, row in enumerate(x.itertuples()):
                if t % 24 == 0: check_deadline(f"storage.{region}", t)
                comp = _flow_components(float(row.FacilityLoad_MW), float(row.ScenarioRenewable_MW), charge, discharge, export_limits[t])
                buy, sell = np.asarray(comp["GridPurchase_MW"], float), np.asarray(comp["GridSell_MW"], float)
                curtail, net = np.asarray(comp["RenewableCurtailment_MW"], float), np.asarray(comp["NetGrid_MW"], float)
                feasible = transition_ok & np.asarray(comp["valid_no_discharge_dump"], bool) & (buy <= float(s["MaxGridImport_MW"]) + TOL)
                step_cost = buy * float(row.ScenarioPrice) - sell * float(row.ScenarioSellPrice)
                step_carbon = buy * float(row.CarbonIntensity_tCO2_per_MWh)
                if objective == "cost_min": step = step_cost
                elif objective == "carbon_min": step = step_carbon * 1000.0
                elif objective == "renewable_max": step = curtail * 1000.0 + np.maximum(step_cost, 0.0) * 1e-6
                elif objective == "peak_min": step = np.maximum(net, 0.0) ** 2
                else: step = step_cost / 1000.0 + step_carbon * 50.0 + curtail * 10.0 + np.maximum(net, 0.0) ** 2 * 0.01
                total = dp[:, None] + np.where(feasible, step, np.inf)
                best_prev = np.argmin(total, axis=0)
                dp = total[best_prev, np.arange(n)]
                predecessors[t] = best_prev
                if not np.isfinite(dp).any(): raise RuntimeError(f"CON-Q3-1: {region} {objective} DP无可行状态")
            final_index = initial_index if terminal_mode == "initial" else int(np.argmin(dp))
            if not np.isfinite(dp[final_index]): raise RuntimeError(f"CON-Q3-2: {region} {objective}终端状态不可达")
            path = np.empty(len(x), dtype=np.int16); current_index = final_index
            for t in range(len(x)-1, -1, -1):
                path[t] = current_index; current_index = int(predecessors[t, current_index])

        previous_soc = initial
        for t, row in enumerate(x.itertuples()):
            soc = float(states[int(path[t])]); delta = soc - previous_soc
            charge_value = max(delta, 0.0) / float(s["ChargeEfficiency"])
            discharge_value = max(-delta, 0.0) * float(s["DischargeEfficiency"])
            comp = _flow_components(float(row.FacilityLoad_MW), float(row.ScenarioRenewable_MW), charge_value, discharge_value, export_limits[t])
            if not bool(comp.pop("valid_no_discharge_dump")): raise RuntimeError(f"CON-Q3-8: {region}/{t} 放电弃置")
            record = {"Region": region, "Hour": int(row.Hour), "FacilityLoad_MW": float(row.FacilityLoad_MW), "AvailableRenewable_MW": float(row.ScenarioRenewable_MW), "SOC_MWh": soc, "GridExportLimit_MW": export_limits[t], "Price": float(row.ScenarioPrice), "SellPrice": float(row.ScenarioSellPrice), "CarbonIntensity": float(row.CarbonIntensity_tCO2_per_MWh)}
            record.update({key: float(np.asarray(value)) for key, value in comp.items()})
            record["RenewableUse_MW"] = record["RenewableDirectUse_MW"]
            record["Curtailment_MW"] = record["RenewableCurtailment_MW"]
            records.append(record); previous_soc = soc
    return pd.DataFrame(records)
...
def storage_local_search(d, facility_override=None, max_candidates=12000):
    ...
    for name in policy_names:
        ...
        diagnostics = _ramp_diagnostics(flow)
        trajectory_hash = _trajectory_hash(flow)
        duplicate_of = seen_hashes.get(trajectory_hash)
        if duplicate_of is None: seen_hashes[trajectory_hash] = name
        efc_rows, efc_total = equivalent_cycles(d, flow)
        source = energy_source_audit(flow, name)
        rows.append({
            "policy": name, "state_count": SOC_STATE_COUNT, "cost_CNY": cost, "carbon_tCO2": carbon,
            "renewable_utilization": renewable, "peak_net_grid_purchase_MW": peak, "peak_MW": peak,
            "net_grid_ramp_MW": ramp, "ramp_MW": ramp, **diagnostics,
            "charge_MWh": float(flow["Charge_MW"].sum()), "discharge_MWh": float(flow["Discharge_MW"].sum()),
            "renewable_charge_MWh": float(flow["RenewableCharge_MW"].sum()), "grid_charge_MWh": float(flow["GridCharge_MW"].sum()),
            "renewable_sell_MWh": float(flow["RenewableSell_MW"].sum()), "storage_sell_MWh": float(flow["StorageSell_MW"].sum()),
            "internal_throughput_MWh": float(efc_rows["internal_throughput_MWh"].sum()),
            "EFC": efc_total, "equivalent_full_cycles_sum": efc_total,
            "max_energy_balance_abs_residual_MW": float(source["max_energy_balance_abs_residual_MW"].max()),
            "max_source_decomposition_abs_residual_MW": float(source["max_source_decomposition_abs_residual_MW"].max()),
            "trajectory_sha256": trajectory_hash, "duplicate_of": duplicate_of,
        })
    for row, flag in zip(rows, _pareto_nondominated(rows)): row["nondominated"] = bool(flag)
    metric_names = ["cost_CNY", "carbon_tCO2", "renewable_utilization", "peak_net_grid_purchase_MW", "net_grid_ramp_MW"]
    weights = np.asarray([0.30, 0.25, 0.15, 0.15, 0.15], dtype=float)
    eligible = [i for i,row in enumerate(rows) if row["nondominated"] and row["duplicate_of"] is None]
    matrix = np.asarray([[rows[i]["cost_CNY"],rows[i]["carbon_tCO2"],-rows[i]["renewable_utilization"],rows[i]["peak_net_grid_purchase_MW"],rows[i]["net_grid_ramp_MW"]] for i in eligible],float)
    ideal,worst=matrix.min(axis=0),matrix.max(axis=0); span=np.where(worst-ideal>1e-12,worst-ideal,1.0); normalized=(matrix-ideal)/span
    distances=np.sqrt(np.sum(weights*normalized**2,axis=1))
    for row in rows: row.update({"normalized_cost":np.nan,"normalized_carbon":np.nan,"normalized_renewable_loss":np.nan,"normalized_peak":np.nan,"normalized_ramp":np.nan,"ideal_distance":np.nan,"decision_eligible":False})
    for local,row_index in enumerate(eligible): rows[row_index].update({"normalized_cost":normalized[local,0],"normalized_carbon":normalized[local,1],"normalized_renewable_loss":normalized[local,2],"normalized_peak":normalized[local,3],"normalized_ramp":normalized[local,4],"ideal_distance":distances[local],"decision_eligible":True})
    chosen_local=min(range(len(eligible)),key=lambda i:(float(distances[i]),rows[eligible[i]]["policy"])); selected_name=rows[eligible[chosen_local]]["policy"]
    base=rows[0]; final=next(row for row in rows if row["policy"]==selected_name); trace=pd.DataFrame(rows)
    summary={"status":"pareto_selected","checked_candidates":len(rows),"unique_trajectories":len(seen_hashes),"accepted_improvements":int(selected_name!="no_action"),"selected_policy":selected_name,"state_count":SOC_STATE_COUNT,"decision_method":"weighted_minmax_ideal_point_distance","decision_weights":json.dumps(dict(zip(metric_names,weights)),ensure_ascii=False,sort_keys=True),"decision_ideal_point":json.dumps(ideal.tolist()),"selection_reason":f"在全部不重复非支配策略中按修正来源与净电网指标的理想点加权距离最小({float(distances[chosen_local]):.12g})","selected_ideal_distance":float(distances[chosen_local]),"baseline_cost_CNY":base["cost_CNY"],"final_cost_CNY":final["cost_CNY"],"baseline_carbon_tCO2":base["carbon_tCO2"],"final_carbon_tCO2":final["carbon_tCO2"],"baseline_peak_MW":base["peak_MW"],"final_peak_MW":final["peak_MW"],"baseline_ramp_MW":base["ramp_MW"],"final_ramp_MW":final["ramp_MW"],"charge_MWh":final["charge_MWh"],"discharge_MWh":final["discharge_MWh"],"renewable_charge_MWh":final["renewable_charge_MWh"],"grid_charge_MWh":final["grid_charge_MWh"],"renewable_sell_MWh":final["renewable_sell_MWh"],"storage_sell_MWh":final["storage_sell_MWh"],"internal_throughput_MWh":final["internal_throughput_MWh"],"equivalent_full_cycles_sum":final["EFC"],"termination_status":"candidate_space_exhausted"}
    Q3_POLICY_RESULTS={name:flows[name] for name in policy_names}
    relaxed=_storage_dp_policy(d,"cost_min",facility_override=facility_override,terminal_mode="free",state_count=SOC_STATE_COUNT)
    Q3_TERMINAL_SENSITIVITY={"interpretation":"题面未给出退化成本或循环寿命参数；本项仅为敏感性，不是官方参数","facility_source":"formal_q2_balanced","initial_equal":next(row for row in rows if row["policy"]=="cost_min"),"terminal_free":{"metrics":metrics_energy(relaxed),"terminal_soc":relaxed.groupby("Region").tail(1)[["Region","SOC_MWh"]].to_dict("records")}}
    return flows[selected_name], summary, trace

def run_soc_grid_sensitivity(d, selected_policy, facility_override=None):
    rows=[]
    for state_count in SOC_SENSITIVITY_STATE_COUNTS:
        check_deadline("q3.grid_sensitivity",state_count); started=time.monotonic()
        flow=_storage_dp_policy(d,selected_policy,facility_override=facility_override,state_count=state_count)
        audit=audit_storage_policy(d,flow,"CON-Q3-1"); cost,carbon,renewable,peak,ramp=metrics_energy(flow)
        efc_rows,cycles=equivalent_cycles(d,flow)
        rows.append({"state_count":state_count,"selected_policy":selected_policy,"cost_CNY":cost,"carbon_tCO2":carbon,"renewable_utilization":renewable,"peak_net_grid_purchase_MW":peak,"peak_grid_purchase_MW":peak,"net_grid_ramp_MW":ramp,"ramp_MW":ramp,"runtime_seconds":time.monotonic()-started,"internal_throughput_MWh":float(efc_rows.internal_throughput_MWh.sum()),"equivalent_full_cycles_sum":cycles,"max_constraint_residual":float(np.maximum(audit.drop(columns="Region").to_numpy(float),0.0).max()),"official_configuration":state_count==SOC_STATE_COUNT,"interpretation":"理想设备情景；EFC按可用SOC区间；题面未给退化成本和循环上限"})
    frame=pd.DataFrame(rows); reference=frame.loc[frame.state_count.eq(max(SOC_SENSITIVITY_STATE_COUNTS))].iloc[0]
    for metric in ["cost_CNY","carbon_tCO2","renewable_utilization","peak_net_grid_purchase_MW","net_grid_ramp_MW","equivalent_full_cycles_sum"]:
        frame[f"{metric}_relative_error_vs_31"]=(frame[metric]-float(reference[metric])).abs()/max(abs(float(reference[metric])),1.0)
    return frame
...
def run_q4_scenarios(d, task_candidates, joint_baseline):
    global Q4_SCENARIO_FLOWS, Q4_CARBON_CANDIDATE_BANK_ROWS, Q4_CARBON_NESTED_AUDIT_ROWS
    Q4_SCENARIO_FLOWS, Q4_CARBON_CANDIDATE_BANK_ROWS, Q4_CARBON_NESTED_AUDIT_ROWS = {}, [], []
    ...
    scenario_specs = [
        ...
        ("price_spread", "price_scale", 0.75, None), ("price_spread", "price_scale", 1.0, None), ("price_spread", "price_scale", 1.25, None),
        ("sell_mechanism", "sell_scale", 0.0, None), ("sell_mechanism", "sell_scale", 0.5, None), ("sell_mechanism", "sell_scale", 1.0, None),
        ("renewable_level", "renewable_scale", 0.8, None), ("renewable_level", "renewable_scale", 1.0, None), ("renewable_level", "renewable_scale", 1.2, None),
        ("renewable_volatility", "seed", 2026.0, 2026), ("renewable_volatility", "seed", 2027.0, 2027), ("renewable_volatility", "seed", 2028.0, 2028),
    ]
    reference_metrics = metrics_energy(joint_baseline)
    reference_service = _service_detail(baseline_schedule, workload)
    baseline_vector = np.asarray([reference_metrics[0], reference_metrics[1], -reference_metrics[2], reference_metrics[3], reference_metrics[4], reference_service["network_latency_ms"], reference_service["mean_wait_h"]], float)
    normal_carbon_reference = float(reference_metrics[1]); stress_mode = bool(normal_carbon_reference <= TOL)
    if stress_mode:
        ...
        pressure_flow, _, _ = _select_q4_storage_policy(pressure_variant, pressure_no_storage[["Region","Hour","FacilityLoad_MW"]])
        carbon_reference = float(metrics_energy(pressure_flow)[1])
    else: carbon_reference = normal_carbon_reference
    formal_price_hash = hashlib.sha256(d["region"][["Region","Hour","ElectricityPrice_CNY_per_MWh"]].sort_values(["Region","Hour"]).to_csv(index=False).encode("utf-8")).hexdigest()
    rows, trace_rows, checked_total = [], [], 0
    inherited_entries = []

    def evaluate_bank_candidate(variant, schedule, policy, level, inherited_from_level, origin):
        no_storage, _, _, _ = no_storage_energy(variant, schedule)
        flow = _storage_dp_policy(variant, policy, facility_override=no_storage[["Region","Hour","FacilityLoad_MW"]], state_count=SOC_STATE_COUNT)
        audit_storage_policy(variant, flow, "CON-Q4-1")
        feasible, used_gpu = schedule_candidate_feasible(variant, workload, schedule)
        if not feasible: return None
        task_audit = build_constraint_audit(variant, workload, schedule, used_gpu)
        if not task_audit["constraint_satisfied"].all(): return None
        metrics = metrics_energy(flow); service = _service_detail(schedule, workload)
        schedule_hash, trajectory_hash = _schedule_hash(schedule), _trajectory_hash(flow)
        cid = hashlib.sha256(f"{schedule_hash}|{trajectory_hash}|{policy}".encode()).hexdigest()
        diagnostics = _ramp_diagnostics(flow)
        return {"CandidateID":cid,"carbon_level":float(level),"inherited_from_level":inherited_from_level,"origin":origin,"storage_policy":policy,"schedule_sha256":schedule_hash,"storage_trajectory_sha256":trajectory_hash,"cost_CNY":metrics[0],"carbon_tCO2":metrics[1],"renewable_utilization":metrics[2],"peak_net_grid_purchase_MW":metrics[3],"net_grid_ramp_MW":metrics[4],**diagnostics,"network_latency_ms":service["network_latency_ms"],"service_quality":service["service_quality"],"mean_wait_h":service["mean_wait_h"],"realtime_sla_rate":service["realtime_sla_rate"],"on_time_rate":service["on_time_rate"],"hard_constraints_satisfied":True,"schedule":schedule.copy(),"flow":flow,"metrics":metrics,"service":service}
    ...
    for scenario_index, (scenario_type, parameter, level, seed) in enumerate(scenario_specs):
        ...
        if scenario_type == "carbon_constraint" and stress_mode:
            variant = _scenario_variant(d, "renewable_shortage_pressure", 0.8); baseline_source = "renewable_shortage_pressure_scale_0.8"
        else:
            variant = _scenario_variant(d, scenario_type, level, seed); baseline_source = "normal_joint_baseline"
        carbon_target = (1.0-float(level))*carbon_reference if scenario_type=="carbon_constraint" else None
        current_schedule = baseline_schedule.copy()
        current_no_storage, _, _, _ = no_storage_energy(variant,current_schedule)
        current_flow,current_policy_summary,_ = _select_q4_storage_policy(variant,current_no_storage[["Region","Hour","FacilityLoad_MW"]],carbon_target=carbon_target)
        current_metrics=metrics_energy(current_flow); current_service=_service_detail(current_schedule,workload)
        current_score,_=_q4_joint_score(current_metrics,current_service,baseline_vector,carbon_target)
        no_improvement=rounds=accepted_rounds=scenario_checked=0
        for round_no in range(1,Q4_MAX_ROUNDS+1):
            check_deadline(f"q4.{scenario_type}.round",round_no); rounds=round_no
            signal_variant={key:value.copy() for key,value in variant.items()}; signal_rt=signal_variant["region"].copy().sort_values(["Region","Hour"]).reset_index(drop=True)
            storage_signal=current_flow.sort_values(["Region","Hour"]).reset_index(drop=True)
            signal_rt["SearchSignal_CNY_per_MWh"]=(signal_rt["ElectricityPrice_CNY_per_MWh"].to_numpy(float)+200.0*signal_rt["CarbonIntensity_tCO2_per_MWh"].to_numpy(float)+5.0*storage_signal["GridPurchase_MW"].to_numpy(float)-5.0*storage_signal["RenewableCurtailment_MW"].to_numpy(float))
            signal_variant["region"]=signal_rt
            candidate_schedule,_,candidate_summary,_,candidate_audit=_optimize_q4_task_neighborhood(signal_variant,workload,current_schedule,Q4_TASK_NEIGHBOR_LIMIT,scenario_index*131+round_no*977)
            scenario_checked+=int(candidate_summary["checked_candidates"]); checked_total+=int(candidate_summary["checked_candidates"])
            candidate_no_storage,_,_,_=no_storage_energy(variant,candidate_schedule)
            candidate_flow,candidate_policy_summary,_=_select_q4_storage_policy(variant,candidate_no_storage[["Region","Hour","FacilityLoad_MW"]],carbon_target=carbon_target)
            candidate_metrics=metrics_energy(candidate_flow); candidate_service=_service_detail(candidate_schedule,workload); candidate_score,_=_q4_joint_score(candidate_metrics,candidate_service,baseline_vector,carbon_target)
            improvement=current_score-candidate_score; accepted=bool(scenario_type!="baseline" and improvement>Q4_IMPROVEMENT_TOL)
            before_hash=_schedule_hash(current_schedule); score_before=current_score
            if accepted:
                current_schedule,current_flow,current_metrics,current_service,current_score,current_policy_summary=candidate_schedule,candidate_flow,candidate_metrics,candidate_service,candidate_score,candidate_policy_summary
                accepted_rounds+=1; no_improvement=0
            else: no_improvement+=1
            trace_rows.append({"scenario_type":scenario_type,"parameter":parameter,"state_count":SOC_STATE_COUNT,"storage_policy":candidate_policy_summary["selected_policy"],"storage_policy_candidate_count":candidate_policy_summary["storage_policy_candidate_count"],"level":level,"seed":seed,"alternation_round":round_no,"task_candidate_budget":Q4_TASK_NEIGHBOR_LIMIT,"task_candidates_checked":candidate_summary["checked_candidates"],"unique_tasks_visited":candidate_summary["unique_tasks_visited"],"local_task_moves_accepted":candidate_summary["accepted_improvements"],"full_task_audit_passed":bool(candidate_audit["constraint_satisfied"].all()),"incumbent_schedule_before_sha256":before_hash,"candidate_schedule_sha256":_schedule_hash(candidate_schedule),"storage_trajectory_sha256":_trajectory_hash(candidate_flow),"joint_score_before":score_before,"joint_score_candidate":candidate_score,"improvement":improvement,"accepted":accepted,"incumbent_schedule_after_sha256":_schedule_hash(current_schedule),"neighborhood_rebuilt_from_previous_storage":True,"search_signal":"SearchSignal_CNY_per_MWh = settlement_price + 200*carbon + 5*purchase - 5*curtailment","settlement_price":"ElectricityPrice_CNY_per_MWh unchanged in formal variant","settlement_price_sha256":formal_price_hash})
            if no_improvement>=Q4_NO_IMPROVEMENT_ROUNDS: break

        if scenario_type=="carbon_constraint":
            bank_candidates=[]; schedule_sources=[(baseline_schedule,None,"normal_joint_baseline"),(current_schedule,float(level),"current_alternating_search")]
            schedule_sources += [(entry["schedule"],entry["carbon_level"],"inherited_looser_level") for entry in inherited_entries]
            seen=set()
            for schedule,from_level,origin in schedule_sources:
                for policy in Q4_STORAGE_POLICIES:
                    candidate=evaluate_bank_candidate(variant,schedule,policy,level,from_level,origin)
                    if candidate is not None and candidate["CandidateID"] not in seen: seen.add(candidate["CandidateID"]); bank_candidates.append(candidate)
            feasible_bank=[c for c in bank_candidates if c["carbon_tCO2"]<=carbon_target+TOL]
            if feasible_bank:
                for c in feasible_bank: c["joint_score"],_=_q4_joint_score(c["metrics"],c["service"],baseline_vector,carbon_target)
                chosen=min(feasible_bank,key=lambda c:(c["joint_score"],c["CandidateID"]))
                current_schedule,current_flow,current_metrics,current_service=chosen["schedule"],chosen["flow"],chosen["metrics"],chosen["service"]
                current_policy_summary={"selected_policy":chosen["storage_policy"],"storage_policy_candidate_count":len(bank_candidates),"decision_method":"nested_carbon_candidate_bank_current_input_recalculation"}
                status="completed"
            else:
                chosen=None; status="target_not_found_within_declared_search"
            for c in bank_candidates:
                selected=bool(chosen is not None and c["CandidateID"]==chosen["CandidateID"])
                Q4_CARBON_CANDIDATE_BANK_ROWS.append({k:v for k,v in c.items() if k not in {"schedule","flow","metrics","service"}} | {"carbon_target_tCO2":carbon_target,"feasible_for_current_target":bool(c["carbon_tCO2"]<=carbon_target+TOL),"selected":selected,"reject_reason":"selected" if selected else ("carbon_target_exceeded" if c["carbon_tCO2"]>carbon_target+TOL else "higher_joint_score")})
            inherited_entries.extend({"schedule":c["schedule"].copy(),"carbon_level":float(level),"CandidateID":c["CandidateID"]} for c in bank_candidates)
            inherited_entries=list({(_schedule_hash(e["schedule"]),e["carbon_level"]):e for e in inherited_entries}.values())
            looser_feasible=[c for c in bank_candidates if c["inherited_from_level"] is not None and float(c["inherited_from_level"])<float(level) and c["carbon_tCO2"]<=carbon_target+TOL]
            monotonic_ok=not looser_feasible or any(c["feasible_for_current_target"] for c in Q4_CARBON_CANDIDATE_BANK_ROWS if c["carbon_level"]==float(level) and c["inherited_from_level"] is not None and float(c["inherited_from_level"])<float(level))
            Q4_CARBON_NESTED_AUDIT_ROWS.append({"carbon_level":float(level),"carbon_target_tCO2":carbon_target,"candidate_count":len(bank_candidates),"inherited_candidate_count":sum(c["inherited_from_level"] is not None and float(c["inherited_from_level"])<float(level) for c in bank_candidates),"feasible_candidate_count":len(feasible_bank),"known_looser_candidate_already_feasible":bool(looser_feasible),"monotonic_inheritance_passed":bool(monotonic_ok),"selected_candidate":chosen["CandidateID"] if chosen else None,"status":status})
            if not monotonic_ok: raise RuntimeError("CON-Q4-8: 更严碳目标漏掉已知可行较松候选")
        else: status="completed"

        final_feasible,final_used_gpu=schedule_candidate_feasible(variant,workload,current_schedule)
        if not final_feasible: raise RuntimeError("CON-Q4-1: 情景最终任务方案全量审计失败")
        final_audit=build_constraint_audit(variant,workload,current_schedule,final_used_gpu)
        if not final_audit["constraint_satisfied"].all(): raise RuntimeError("CON-Q4-1: 情景最终任务约束残差失败")
        audit_storage_policy(variant,current_flow,"CON-Q4-1")
        current_metrics=metrics_energy(current_flow); current_service=_service_detail(current_schedule,workload)
        mechanism_active,active_constraint,purchase,selling=_q4_mechanism_state(scenario_type,level,current_flow,carbon_target,carbon_reference,stress_mode)
        diagnostics=_ramp_diagnostics(current_flow); scenario_key=f"{scenario_index:02d}_{scenario_type}_{level}_{seed}"
        Q4_SCENARIO_FLOWS[scenario_key]=current_flow.assign(scenario_type=scenario_type,parameter=parameter,level=level,seed=seed,scenario_key=scenario_key)
        rows.append({"scenario_type":scenario_type,"parameter":parameter,"state_count":SOC_STATE_COUNT,"storage_policy":current_policy_summary["selected_policy"],"storage_policy_candidate_count":current_policy_summary["storage_policy_candidate_count"],"storage_policy_selection_rule":current_policy_summary["decision_method"],"baseline_source":baseline_source,"carbon_reference_mode":"renewable_shortage_pressure" if scenario_type=="carbon_constraint" and stress_mode else "normal_baseline","carbon_reference_tCO2":carbon_reference if scenario_type=="carbon_constraint" else np.nan,"carbon_target_tCO2":carbon_target,"level":level,"seed":seed,"status":status,"alternation_rounds":rounds,"accepted_rounds":accepted_rounds,"checked_candidates":scenario_checked,"common_start_count":1,"fixed_candidate_set":False,"dynamic_neighborhood_rebuilt_each_round":True,"selected_candidate":_schedule_hash(current_schedule),"storage_trajectory_sha256":_trajectory_hash(current_flow),"migration_count":current_service["migration_count"],"task_search_status":"alternating_improved" if accepted_rounds else "searched_no_improvement","cost_CNY":current_metrics[0],"carbon_tCO2":current_metrics[1],"network_latency_ms":current_service["network_latency_ms"],"service_quality":current_service["service_quality"],"realtime_sla_rate":current_service["realtime_sla_rate"],"on_time_rate":current_service["on_time_rate"],"max_wait_h":current_service["max_wait_h"],"p95_wait_h":current_service["p95_wait_h"],"mean_wait_h":current_service["mean_wait_h"],"renewable_utilization":current_metrics[2],"peak_net_grid_purchase_MW":current_metrics[3],"peak_grid_purchase_MW":current_metrics[3],"net_grid_ramp_MW":current_metrics[4],"ramp_MW":current_metrics[4],**diagnostics,"grid_purchase_MWh":purchase,"grid_sell_MWh":selling,"mechanism_active":mechanism_active,"active_constraint":active_constraint,"constraints_satisfied":1,"non_Pareto":True,"result_level":"scenario-feasible","termination_status":"no_improvement" if no_improvement>=Q4_NO_IMPROVEMENT_ROUNDS else "max_iterations_reached","search_signal_field":"SearchSignal_CNY_per_MWh","settlement_price_field":"ElectricityPrice_CNY_per_MWh","settlement_price_sha256":formal_price_hash})
    frame=pd.DataFrame(rows); family_summary=_summarize_q4_families(frame,carbon_reference,stress_mode)
...
def main():
    ...
    q3_source_audit = pd.concat([energy_source_audit(flow, policy) for policy, flow in q3_policy_flows.items()], ignore_index=True)
    q3_source_audit.to_csv(RESULT_DIR / "q3_energy_source_audit.csv", index=False)
    q3_selected_source = energy_source_audit(q3_flow, q3_search["selected_policy"])
    with open(RESULT_DIR / "q3_renewable_accounting.json", "w", encoding="utf-8") as f:
        json.dump({"formula":"sum(RenewableDirectUse_MW+RenewableCharge_MW+RenewableSell_MW)/sum(AvailableRenewable_MW)","selected_policy":q3_search["selected_policy"],"renewable_direct_use_MWh":float(q3_flow["RenewableDirectUse_MW"].sum()),"renewable_charge_MWh":float(q3_flow["RenewableCharge_MW"].sum()),"grid_charge_MWh":float(q3_flow["GridCharge_MW"].sum()),"renewable_sell_MWh":float(q3_flow["RenewableSell_MW"].sum()),"storage_sell_MWh":float(q3_flow["StorageSell_MW"].sum()),"renewable_curtailment_MWh":float(q3_flow["RenewableCurtailment_MW"].sum()),"renewable_utilization":q3_re,"max_energy_balance_abs_residual_MW":float(q3_selected_source["max_energy_balance_abs_residual_MW"].max()),"max_source_decomposition_abs_residual_MW":float(q3_selected_source["max_source_decomposition_abs_residual_MW"].max())},f,ensure_ascii=False,indent=2)
    q3_efc_rows, q3_efc_total = equivalent_cycles(d, q3_flow)
    q3_efc_rows.to_csv(RESULT_DIR / "q3_equivalent_cycles_by_region.csv", index=False)
    with open(RESULT_DIR / "q3_equivalent_cycles_audit.json", "w", encoding="utf-8") as f:
        json.dump({"formula":"sum_t(eta_charge*Charge_MW+Discharge_MW/eta_discharge)/(2*(SOC_max-SOC_min))","regions":q3_efc_rows.to_dict("records"),"EFC_total":q3_efc_total,"status":"pass"},f,ensure_ascii=False,indent=2)
    q3_trace.to_csv(RESULT_DIR / "q3_metric_recomputation_audit.csv", index=False)
    if len(Q4_SCENARIO_FLOWS) != 17:
        raise RuntimeError(f"CON-Q4-4: 缺少Q4逐时情景流 {len(Q4_SCENARIO_FLOWS)}/17")
    q4_all_scenario_flow = pd.concat(Q4_SCENARIO_FLOWS.values(), ignore_index=True)
    q4_all_scenario_flow.to_csv(RESULT_DIR / "q4_all_scenario_energy.csv", index=False)
    q4_source_frames = [energy_source_audit(flow, key).assign(scenario_key=key) for key, flow in Q4_SCENARIO_FLOWS.items()]
    q4_source_audit = pd.concat(q4_source_frames, ignore_index=True)
    q4_source_audit.to_csv(RESULT_DIR / "q4_energy_source_audit.csv", index=False)
    with open(RESULT_DIR / "q4_renewable_accounting.json", "w", encoding="utf-8") as f:
        json.dump({"formula":"sum(RenewableDirectUse_MW+RenewableCharge_MW+RenewableSell_MW)/sum(AvailableRenewable_MW)","scenario_count":len(Q4_SCENARIO_FLOWS),"rows":q4_source_audit.to_dict("records"),"max_energy_balance_abs_residual_MW":float(q4_source_audit["max_energy_balance_abs_residual_MW"].max()),"max_source_decomposition_abs_residual_MW":float(q4_source_audit["max_source_decomposition_abs_residual_MW"].max())},f,ensure_ascii=False,indent=2)
    pd.DataFrame(Q4_CARBON_CANDIDATE_BANK_ROWS).to_csv(RESULT_DIR / "q4_carbon_candidate_bank.csv", index=False)
    pd.DataFrame(Q4_CARBON_NESTED_AUDIT_ROWS).to_csv(RESULT_DIR / "q4_carbon_nested_feasibility_audit.csv", index=False)
    with open(RESULT_DIR / "q4_carbon_constraint_provenance.json", "w", encoding="utf-8") as f:
        json.dump({"levels":[0.0,0.1,0.2,0.3],"order":"loose_to_strict","candidate_bank_file":"q4_carbon_candidate_bank.csv","nested_audit_file":"q4_carbon_nested_feasibility_audit.csv","settlement_price_field":"ElectricityPrice_CNY_per_MWh","search_signal_field":"SearchSignal_CNY_per_MWh","failure_label":"target_not_found_within_declared_search","mathematical_infeasibility_claimed":False},f,ensure_ascii=False,indent=2)
    ...
    save_figure(
        ...
        "问题三所选策略的六区域 SOC 轨迹", "时刻 / h", "SOC / MWh", manifest
    )
    q4_soc_plot = q4_flow.pivot(index="Hour", columns="Region", values="SOC_MWh").reset_index()
    save_figure(
        q4_soc_plot, "q4_joint_storage_soc.png", "line", "Hour", REGIONS,
        "问题四联合基准的六区域 SOC 轨迹", "时刻 / h", "SOC / MWh", manifest
    )
    figure_data_path = FIGURE_DATA_DIR / "fig_2_storage_soc.csv"
    q3_sorted = soc_plot[["Hour"] + REGIONS].sort_values("Hour").reset_index(drop=True)
    q4_sorted = q4_soc_plot[["Hour"] + REGIONS].sort_values("Hour").reset_index(drop=True)
    figure_sorted = pd.read_csv(figure_data_path)[["Hour"] + REGIONS].sort_values("Hour").reset_index(drop=True)
    figure_q3_diff = float(np.abs(figure_sorted[REGIONS].to_numpy(float)-q3_sorted[REGIONS].to_numpy(float)).max())
    figure_q4_equal = bool(np.allclose(figure_sorted[REGIONS],q4_sorted[REGIONS],rtol=0.0,atol=1e-12))
    if figure_q3_diff > TOL or (not np.allclose(q3_sorted[REGIONS],q4_sorted[REGIONS],rtol=0.0,atol=1e-12) and figure_q4_equal):
        raise RuntimeError("FIG-Q3-001: Q3 SOC图来源串线")
    with open(RESULT_DIR / "q3_soc_figure_provenance.json", "w", encoding="utf-8") as f:
        json.dump({"selected_q3_policy":q3_search["selected_policy"],"q3_flow_file":"q3_storage_energy.csv","q3_flow_sha256":sha256_file(RESULT_DIR / "q3_storage_energy.csv"),"figure_data_file":"figure_data/fig_2_storage_soc.csv","figure_data_sha256":sha256_file(figure_data_path),"q4_flow_sha256":sha256_file(RESULT_DIR / "q4_joint_storage_energy.csv"),"figure_data_vs_q3_flow_max_abs_difference_MWh":figure_q3_diff,"figure_data_equals_q4_flow":figure_q4_equal,"status":"pass"},f,ensure_ascii=False,indent=2)
