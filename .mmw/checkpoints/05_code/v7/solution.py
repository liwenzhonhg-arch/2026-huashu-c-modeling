...
def no_storage_energy(d, schedule):
    ...
    export_limits = []
    gpu_rows = d["gpu"].set_index("Region")
    for _, row in rt.iterrows():
        if "MaxGridExport_MW" in row.index and pd.notna(row["MaxGridExport_MW"]):
            export_limits.append(float(row["MaxGridExport_MW"]))
        elif "MaxGridExport_MW" in gpu_rows.columns:
            export_limits.append(float(gpu_rows.loc[row["Region"], "MaxGridExport_MW"]))
        else:
            raise RuntimeError(
                "CON-Q2-3: actual=missing, threshold=finite MaxGridExport_MW"
            )
