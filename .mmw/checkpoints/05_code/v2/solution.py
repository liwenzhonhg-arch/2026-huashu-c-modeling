...
# ponytail: bounded local search preserves the full feasible incumbent; increase only if the 285 s budget has measured headroom.
MAX_LOCAL_IMPROVEMENTS = 10000
...
def improve_feasible_incumbent(
    ...
    for task_index, row in enumerate(order.itertuples(index=False), start=1):
        ...
        old_overlaps = np.minimum(
            old_start + duration, old_hours + 1.0
        ) - np.maximum(old_start, old_hours)
        ...
        for target in REGIONS:
            ...
            if latency_value > float(row.MaxLatency_ms) + TOL:
                continue
            region_array = region_arrays[target]
            for start in shortlist_starts(
                mode, arrival, max_start, region_array
            ):
                if start + duration > latest + TOL:
                    continue
                score, hours, overlaps = candidate_score(
                    mode, start, arrival, duration, demand, task_power,
                    latency_value, region_array,
                )
                ranked.append((
        ...
        old_score, _, _ = candidate_score(
            mode, old_start, arrival, duration, demand, task_power,
            old_latency, region_arrays[old_target],
        )
        ranked.append((
            old_score, old_start, old_target, old_latency,
            old_hours, old_overlaps,
        ))
        ...
        selected = next((
            ...
            if candidate_is_feasible(
                candidate, demand, task_power,
                gpu_used, ai_power, region_arrays,
            )
        ), None)
        if selected is None:
            raise RuntimeError(
                f"incumbent恢复失败: TaskID={task_id}"
            )

        score, start, target, latency_value, hours, overlaps = selected
        gpu_used[target][hours] += demand * overlaps
        ai_power[target][hours] += demand * task_power * overlaps
        if (target, start) != (old_target, old_start) and score < old_score - TOL:
            improved += 1
        schedule.at[task_id, "ExecutionRegion"] = target
        schedule.at[task_id, "StartHour"] = float(start)
        schedule.at[task_id, "FinishHour"] = float(start + duration)
        schedule.at[task_id, "WaitingHour"] = float(start - arrival)
