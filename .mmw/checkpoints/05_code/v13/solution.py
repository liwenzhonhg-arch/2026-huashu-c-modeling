...
# 285秒是合同硬上限。代码使用更早的主动停止点，为完整审计、CSV、JSON和
# 300 dpi图表导出保留确定性时间。不得把搜索运行到外部300秒超时边界。
SEARCH_LIMIT = min(CONTRACT_SEARCH_LIMIT, PROGRAM_START + 270.0)
TOTAL_LIMIT = PROGRAM_START + 300.0
SEED = 2026
TOL = 1e-6
Q4_MOVE_LIMIT = 12000
# 问题二原实现每个候选都会复制50000行调度表，并重新执行50000任务的
# 全量资源与能源复算。20000个候选的最坏复杂度约为10亿任务次，无法满足
# 300秒合同。这里保留真实完整复算，但把本次有限启发式检查集限制为4个。
Q2_MOVE_LIMIT = 4
Q2_TASK_POOL_SIZE = 4
Q4_TASK_NEIGHBOR_LIMIT = 12
...
def greedy_schedule(d, workload):
    ...
    for row in tasks.itertuples():
        if time.monotonic() >= SEARCH_LIMIT:
            ...
            raise RuntimeError(
                f"CON-Q1-7: actual={elapsed:.6f}, threshold=285, "
                "候选扫描达到统一搜索截止"
            )
...
def main():
    ...
    if len(q4_scenarios) != 15:
        fail("CON-Q4-4", len(q4_scenarios), 15, "规定情景未全部生成")
    if q4_checked < 45:
    ...
    if time.monotonic() >= TOTAL_LIMIT:
        fail("CON-Q4-6", time.monotonic() - PROGRAM_START, 300, "输出阶段前已超时")
    ...
    runtime = {
        ...
        "search_evidence": {
            ...
            "q4_scenarios": {
                ...
                "alternation_rounds": 2,
                "required_minimum_checked_candidates": 45,
                ...
                "shared_candidate_limit": Q4_MOVE_LIMIT,
                "all_scenarios_compared_three_task_storage_combinations": bool(
                    (
                        q4_scenarios.loc[
                            q4_scenarios["constraints_satisfied"].eq(1),
                            "checked_candidates",
