...
# 300秒合同。这里保留真实完整复算，但把本次有限启发式检查集限制为8个。
Q2_MOVE_LIMIT = 8
Q2_TASK_POOL_SIZE = 4
...
def build_q4_task_candidate_set(
    ...
    基线任务方案沿用问题二结果；替代方案从同一单任务邻域中选取第一个与
    基线不同、并通过一次50000任务全量硬约束审计的方案。该全量任务审计
    只执行一次，15个情景复用审计结论，但会分别重建各候选的设施负荷和
    储能策略。
    """
    ...
    for row_idx in order:
        ...
        for start in starts:
            ...
            for region in REGIONS:
                ...
                if feasible:
                    alternative = trial
                    alternative_used_gpu = trial_used_gpu
                    break

    if alternative is None:
        raise RuntimeError(
            "CON-Q4-2: actual=1, threshold=2, "
            "未构造出与基线不同的全量任务可行候选"
        )

    alternative_audit = build_constraint_audit(
        d, workload, alternative, alternative_used_gpu
    )
    ...
    candidates = [
        ...
        {
            "candidate_id": "task_alternative_1",
    ...
    evidence = {
        ...
        "full_task_audits": 1,
        "alternative_max_constraint_residual": alternative_residual,
    }
    return candidates, evidence, alternative_audit
    ...
    for scenario_type, parameter, level in scenario_specs:
        if time.monotonic() >= SEARCH_LIMIT:
            rows.append({
                ...
                "selected_candidate": "",
                "migration_count": baseline_migrated,
                "task_search_status": "not_run_budget_exhausted",
                "cost_CNY": baseline_cost,
                "carbon_tCO2": baseline_carbon,
                "network_latency_ms": baseline_latency,
                "service_quality": baseline_service,
        ...
        if len(evaluated) < len(task_candidates):
            rows.append({
                "scenario_type": scenario_type,
                "parameter": parameter,
                "level": level,
                "status": "not_run_budget_exhausted",
                "checked_candidates": len(evaluated),
                "selected_candidate": "",
                "migration_count": baseline_migrated,
                "task_search_status": "incomplete_candidate_comparison",
                "cost_CNY": baseline_cost,
                "carbon_tCO2": baseline_carbon,
                "network_latency_ms": baseline_latency,
                "service_quality": baseline_service,
...
def storage_local_search(d, facility_override=None, max_candidates=12000):
    policies = [
        (0.10, 0.60), (0.10, 0.75), (0.10, 0.90),
        (0.20, 0.60), (0.20, 0.75), (0.20, 0.90),
        (0.30, 0.60), (0.30, 0.75), (0.30, 0.90),
        (0.40, 0.60), (0.40, 0.75), (0.40, 0.90),
    ]
...
def main():
    ...
    (
        ...
        q4_alternative_audit,
    ) = build_q4_task_candidate_set(
        d,
        workload,
        schedule,
        q2_flow,
        max_checked=Q2_MOVE_LIMIT,
    )
    ...
    q4_alternative_audit.to_csv(
        RESULT_DIR / "q4_alternative_task_constraint_audit.csv", index=False
    )
    q4_scenarios.to_csv(
    ...
    results.extend([
        ...
        result_item(
            ...
            "个",
            "统一候选集包含基线和一个通过全量任务审计的不同替代方案",
        ),
        result_item(
            "q4_情景任务候选评价数",
            q4_checked,
            "次",
            "15个情景对统一任务候选集执行的实际完整能源与储能评价总数",
        ),
    ...
    runtime = {
        ...
        "search_evidence": {
            ...
            "q4_scenarios": {
                ...
                "all_completed_scenarios_compared_two_tasks": bool(
                    (
                        q4_scenarios.loc[
                            q4_scenarios["constraints_satisfied"].eq(1),
                            "checked_candidates",
