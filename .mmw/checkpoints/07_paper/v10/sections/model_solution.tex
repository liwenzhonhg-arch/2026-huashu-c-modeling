% MMW-METHOD-CONTRACT: 2b2655f0adcff70f11595005cf622dc5a580d635c4396b70aabb43712f1b9840
\section{模型的建立与求解}
...
实际程序只形成有限候选集：问题二完整复算 $4$ 个单任务候选，问题三审计 $5$ 个规则型储能候选，问题四在每个情景以两轮顺序交替方式比较同一组 $3$ 个任务方案并重建对应储能策略。实际决策规则是“先通过全部硬约束审计，再在实际检查集合中保留代理评分 best-found；审计失败或构造未完成则不输出正式候选”。这些有限候选不足以形成可信 Pareto 前沿，故结果仍只能称为多指标评价下的场景可行候选，而不是加权和最优解、Pareto 前沿或全局最优解。
...
\subsection{方法偏差与适用边界}
% MMW-LIMITATION: L10
\paragraph{方法边界 L10}本题不涉及PDE，PDE和移动热过程函数调用次数为0
