...
实际程序没有形成可供 Pareto 筛选的候选集：问题二至问题四的每个已报告场景只构造一个规则型候选。因此实际决策规则是“先通过全部硬约束审计，再接受该唯一候选；审计失败或构造未完成则不输出正式候选”，并同时报告上述多项指标。故本文的实际结果应称为多指标评价下的场景可行候选，而不是加权和最优解、Pareto 前沿或全局最优解。
