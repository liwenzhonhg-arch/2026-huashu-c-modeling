...
def main():
    ...
    base_carbon_weight = 200.0
    baseline_objective = q2_cost + base_carbon_weight * q2_carbon
    for param, baseline_value in {
        "carbon_weight": base_carbon_weight,
        "price_scale": 1.0,
    }.items():
        plot_rows = []
        for delta in [-20, -10, 10, 20]:
            ...
            changed_objective = (
                q2_cost + param_value * q2_carbon
                if param == "carbon_weight"
                else param_value * q2_cost + base_carbon_weight * q2_carbon
            )
            change = 100.0 * (
                changed_objective - baseline_objective
        ...
        ax.axhline(baseline_objective, linestyle="--", label="基准值")
        ax.set_xlabel("参数扰动 / %")
        ax.set_ylabel("固定调度复合目标 / 元")
        ...
        save_figure(
            ...
            f"{param}灵敏度分析", "参数扰动 / %",
            "固定调度复合目标 / 元", f"{param}扰动下固定q2调度的成本加碳代价."
        )
