...
本节是 v42 的现役实现合同；若前文的历史搜索预算或实现建议与本节冲突，以本节为准。
数学目标、任务唯一分配、容量、时延、截止、能源守恒、SOC、功率、互斥和购售电边界继续使用前文完整公式。

## 问题一：严格闭环预测

对每个区域—任务类型序列独立处理。训练集为 0--2351，验证集为 2352--2375，测试集为
2376--2399。候选为 24 小时季节朴素、168 小时季节朴素，以及
$\epsilon\in\{1.10,1.35,1.75\}$、$\alpha\in\{10^{-4},10^{-3},10^{-2}\}$ 的 Huber 网格。
验证和测试均从各自预测原点递归 24 步：$t-1,t-2,t-3$ 若已进入预测区间，只能使用此前预测值；
不得使用验证或测试真实标签。按验证 RMSE、MAE、WAPE、固定候选序确定性选型，随后用
0--2375 重拟合并闭环预测测试集。WAPE 分母为零时明确标记 unavailable，不参与该项平局判断。
正式任务调度仍使用附件真实任务，不用预测量替换正式需求。

## 问题二：五类增量任务调度

容量—时延贪心仅作为公开 baseline。对 cost_min、carbon_min、renewable_max、service_first、
balanced 五个命名方案分别使用 10000 个快速候选，总上限 50000。一个任务移动只更新受影响的
区域—小时 GPU、IT、设施负荷和购售电边际量，不为每个候选复制 50000 行表。候选包含合法区域迁移
与 arrival、当前、相邻 1/6 小时和 latest-start 时间平移。每个命名方案完成后均对全部 50000 个任务
重新执行唯一分配、实时开工、截止、时延、GPU、IT、设施功率和购电边界审计。

目标必须同时报告成本、碳、新能源利用、网络时延、等待和迁移量。成本另拆分无 AI 基础项和 AI
增量项，固定新能源外送收入不得冒充任务调度收益。正式结果是有限候选 best-found，不具有全局最优证书。

## 问题三：含 no-action 的确定性 SOC 动态规划

no-action 是正式候选。每区域把 $[S_r^{min},S_r^{max}]$ 离散为 21 个状态并显式加入 $S_r^0$，
通过逐时动态规划比较 cost_min、carbon_min、renewable_max、peak_min、ramp_min、balanced；
状态转移严格满足效率、充放电功率、购电、售电、充放互斥和购售互斥。基准终端约束为
$S_{r,2406}=S_r^0$，自由终端仅作为敏感性，不参与正式选择。

对成本、碳、负新能源利用率、峰值和爬坡执行 Pareto 支配检查。若 balanced 被支配，则从非支配集
按到理想点的归一化距离选取；若全部储能轨迹被 no-action 支配，则选择 no-action。相同轨迹哈希只
计作一个独立策略，并报告等效循环次数。

## 问题四：动态邻域的真实交替

每个情景从同一任务基线与真实初始 SOC 独立开始。每轮先固定任务求储能，再利用本轮购电、碳和
弃电边际信号重新生成任务迁移/时间平移邻域；候选任务方案通过 50000 任务全量审计后，再固定新任务
重新求储能。最大 8 轮、连续 2 轮无改进停止、改善容差 $10^{-8}$。每轮 trace 保存前后调度哈希、
储能轨迹哈希、候选覆盖、接受判断与停止原因。

情景包括碳约束、峰谷价差、售电机制、新能源水平，以及种子 2026、2027、2028 的三个新能源波动
序列。每个情景报告实时 SLA、按期完成率、最大/95%/平均等待、网络时延、迁移数和全部能源指标。
结论限定为 scenario-feasible。

## 确定性运行与停止

正式程序默认无墙钟上限。问题二按 50000 个候选停止；问题三按完整七策略集合停止；问题四按每轮
600 候选、最多 8 轮、连续 2 轮无改进停止。只有用户显式设置 MMW_MAX_RUNTIME_SECONDS 时启用
保护性墙钟；触发后必须记录 external_timeout、incomplete=true，并阻断 solve、paper 和 review。
确定性候选/迭代上限表示有限搜索完成，不表示全局最优。


## v43 实际落地与鲁棒性增量

Q2采用五维确定性分层轮转并限制单任务候选数；Q3对不重复非支配策略统一使用权重为0.30/0.25/0.15/0.15/0.15的min-max理想点距离；Q4联合基准与17个情景统一采用21个SOC状态。15/21/31网格仅作为独立敏感性。正式运行默认无墙钟，显式墙钟触发必须输出external_timeout与incomplete。
