...
$\lambda$ 和 Huber 参数 $\delta_H$ 仅由训练集与验证集确定，不能使用测试集或最终优化结果调整。模型确定后用 $168$–$2375$ 重训。
...
式（79a）仅作快速必要条件。每次冻结 $F_j$ 前，还必须以已冻结状态为起点，为所有未提交任务构造一份直至第 2406 小时的完整可行补全证书：补全须满足任务唯一执行、容量、功率、能源守恒、购售电边界、逐时 SOC 递推和 $E_{r,2406}\ge E_r^0$。只有该证书通过确定性残差复核才提交 $F_j$；否则拒绝本轮提交并保留上一份完整可行 incumbent。由此，窗口递推不仅满足乐观必要条件，还保留可验证的剩余时域终端可行性。证书摘要和最大残差写入运行日志。最终方案必须按式（28）–（33）、（53）–（60）、（70）–（72）进行全时域复算。
...
仅当 $N_{launch}\ge1$ 且单候选的保守时间上限能够完整放入 $D_{search}-t_{now}$ 时启动新候选；传给求解器的 `time_limit` 不超过 $\min\{30,D_{search}-t_{now}\}$ s。任何候选不得越过 $D_{search}$；到达搜索截止立即停止新增或继续求解，并丢弃未形成完整可行解的候选。最后 $15$ s 仅用于确定性全时域复核和文件输出。式（91）只是启动上限，不要求机械完成全部 $12$ 个组合。
