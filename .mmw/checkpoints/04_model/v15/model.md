...
第 $2400$–$2405$ 小时不产生新任务，只结清弹性任务；第 $2406$ 小时不执行任务，只进行电力和终端 SOC 结算。所有任务不可抢占、不可拆分、不可中途迁移。
...
| $P_{rt}^{export,max},S_r^{max}$ | 问题三、四的区域外送边界与储能表外送上限 | 可观测参数 | MW |
...
$$
0\le B_{rt}\le P_{rt}^{grid,max}, \tag{26}
$$

$$
0\le S_{rt}\le R_{rt}^{av},
...
$$
W_j=[b_j,e_j],\qquad e_j=\min(b_j+H-1,2406), \tag{36}
$$
...
该式阻止滚动窗口过度放电后再也无法在剩余时段恢复初始 SOC；最终方案必须按式（28）–（33）、（53）–（60）、（70）–（72）进行全时域复算。
...
其中 $w_m\ge0$、$\sum_mw_m=1$，权重必须在查看结果前预声明。
