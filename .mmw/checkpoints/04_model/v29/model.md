...
对每个区域—任务类型训练序列，令 $\sigma_{rk}=\max\{1,1.4826\,\mathrm{MAD}(D_{rk,t})\}$。数学式（16）的 $\lambda$ 与 scikit-learn `HuberRegressor(alpha=...)` 是同一 L2 正则参数。增强预测候选固定为 $(\lambda=\alpha,\delta_H/\sigma_{rk})\in\{(10^{-3},1),(10^{-3},2),(10^{-1},1),(10^{-1},2)\}$，与实际实施一致，不再执行24组合网格。只在 $2352$–$2375$ 验证集上按“可计算 WAPE 最小、MAE 最小、$\lambda$ 较小、$\delta_H/\sigma_{rk}$ 较小”的字典序唯一选型；若该分组 WAPE 不可计算，则从 MAE 开始比较。不能使用测试集或最终优化结果调整。增强模型与两个季节基线的最终选择也固定：对每个区域—任务类型，在固定起点递归验证集上按“可计算 WAPE、MAE、模型优先序”的字典序最小化，完全相同时优先序为 24 小时季节基线、168 小时季节基线、增强回归。若 WAPE 不可计算则从 MAE 比较。仅当增强回归胜出时用 $168$–$2375$ 重训；否则保留胜出的季节基线。
...
否则输出 `autocorrelation_unavailable`。缺失值必须先按成对删除处理；不得用任意常数填充后计算相关系数。 周期候选固定为 $h=2,\ldots,336$；仅保留满足 $ACF_h>0.3$ 且同时大于相邻 $ACF_{h-1},ACF_{h+1}$ 的局部峰。候选中取 ACF 最大者，完全同值取较小 $h$，输出 `period_h`；无候选、样本不足或方差为零时输出 `period_unidentifiable`，不得填造周期。
...
$$
0\le B_{rt}\le G_r^{import,max}, \tag{26}
$$

$$
0\le S_{rt}\le\min(R_{rt}^{av},G_r^{export,max}),
\qquad R_{rt}^{use},K_{rt}\ge0. \tag{27}
$$

附件适用表与题面“区域购售电边界”要求存在口径冲突。为避免产生超过接入能力的工程不可行方案，问题二仅借用 `storage_information.xlsx` 的 `MaxGridImport_MW`、`MaxGridExport_MW` 作为六区域静态电网接入硬边界，不使用 SOC、充放电、效率或 `SellLimit_MW`。因此 $0\le B_{rt}\le G_r^{import,max}$，$0\le S_{rt}\le\min(R_{rt}^{av},G_r^{export,max})$，且不会售出电网购电；这一跨表解释在论文中单独披露。
...
前述各问 MILP 与 ε-约束体系仅是理论 formulation，用于解释目标和物理守恒；它不是本次代码执行契约，也不得在论文中写成已经运行。第 8 节是唯一实际 implementation 合同。受 50,000 个任务与单次 300 s 共享墙钟限制，本次只运行下述确定性启发式，二者必须在论文中分开叙述。

1. **预测**：24 h 与 168 h 季节基线均按固定预测起点递归计算；增强模型只评估四个预声明 Huber-Ridge 代表组合：$(\alpha,\delta_H/\sigma)\in\{(10^{-3},1),(10^{-3},2),(10^{-1},1),(10^{-1},2)\}$，其中 $\sigma=\max\{1,1.4826\,MAD\}$；`max_iter=80`、`tol=10^{-4}`。按验证 WAPE、MAE、简单模型优先序唯一选择，再对 2376–2399 固定起点测试；收敛失败的增强候选丢弃并保留季节基线。
2. **任务调度**：完整 incumbent 的唯一构造顺序为“实时任务优先、LatestFinishHour 升序、EstimatedDuration_min 升序、TaskID 升序”；每个任务先尝试 ArrivalHour，在满足时延的区域中按网络时延、区域名顺序选择第一个容量可行位置；若弹性任务在到达时刻无位置，则按开工小时从 $ArrivalHour$ 到 $\lfloor LatestFinishHour-Duration_h\rfloor$ 升序、同小时按网络时延和区域名升序扫描完整允许域。只有完整允许域均无容量可行位置才停止并输出 `infeasible_incumbent`。实时任务仍只能在 ArrivalHour 开工。构造后复核全部 50,000 个任务，失败则整次停止。问题二、四的待改进集合只含非实时任务，按 $GPU\_Demand\times Duration_h$ 降序、LatestFinishHour 升序、TaskID 升序取前 10,000 个。每次先移除旧占用。对候选逐小时定义设施增量能量 $\Delta E_t=\mathrm{PUE}_rg_ip_k\omega_{it}(s)$ MWh，其中 $p_k$ 为 MW/GPU、$\omega$ 为 h；$E=\sum_t\Delta E_t$ MWh。移除当前任务后，既有固定负荷和其他任务占用下的剩余新能源能量为 $R_{rt}^{res}=\max\{0,R_{rt}^{av}\Delta t-\mathrm{PUE}_r(P_{rt}^{NA}+P_{rt}^{AI,other})\Delta t\}$ MWh。由原始附件固定尺度 $\widetilde p=\operatorname{median}(price)>0$、$\widetilde c=\operatorname{median}(carbon)>0$。定义五个无量纲分量：$z_C=\sum_t\Delta E_tprice_{rt}/\max(1,E\widetilde p)$，$z_E=\sum_t\Delta E_tcarbon_{rt}/\max(1,E\widetilde c)$，$z_R=\sum_t\min(\Delta E_t,R_{rt}^{res})/\max(1,E)$，$z_W=(s-a_i)/\max(1,\lfloor f_i^{late}-d_i\rfloor-a_i)$，$z_L=\ell_{o_ir}/\max(1,\ell_i^{max})$。问题二评分固定为 $score=z_C+z_E-0.5z_R+0.5z_W+0.5z_L$；问题四固定为 $score=z_C+1.5z_E-z_R+z_W+0.5z_L$。按 score、开工小时、区域名升序选择；只有新 score 严格改善且全部任务硬约束仍满足才接受，否则恢复原位置。未搜索任务保持 incumbent 决策。该 score 无量纲，只是固定启发式排序量。
3. **问题二能源流**：无储能，逐时按新能源优先直供，余量在 $MaxGridExport_MW$ 内外送，不足部分在 $MaxGridImport_MW$ 内购电；严格复算成本、购电碳排放、时延、直接消纳率、外送率和退化的统一新能源利用率。只输出一个经验证的启发式候选，不把单点文件称为完整 Pareto 前沿。
4. **问题三储能**：原始 `storage_information.xlsx` 没有 `MaxSOC_MWh` 字段；字段说明明确 `StorageCapacity_MWh` 是“最大可存储电量”，故物理可行域为 $MinSOC_MWh\le E_t\le StorageCapacity_MWh$；实际启发式额外采用保守限制 $E_t\ge InitialSOC$，这是可行域的子集而不是篡改附件下界。逐区域先计算真实购电价的 25%/75% 分位数 $p_{25},p_{75}$，从 `InitialSOC_MWh` 前向递推。每小时依次执行：$R^{use}=\min(P^{fac},R^{av})$，得到 `surplus/deficit`；新能源充电取 `min(surplus, MaxChargePower, (Capacity-E)/eta_c)`；仅当 $price\le p_{25}$ 且 $t<2406$ 时，在剩余充电功率、SOC 空间和购电上限内电网充电；仅当 $price\ge p_{75}$ 且 $t<2406$ 时，放电取 `min(deficit, MaxDischargePower, (E-InitialSOC)*eta_d)`；充电发生时放电固定为0，放电发生时充电固定为0；外送取新能源充电后的剩余新能源与 `SellLimit_MW/MaxGridExport_MW` 的最小值，余量弃电。第2406小时禁止放电且 $E_{2406}\ge InitialSOC$。缺失 `MaxGridImport/Export` 时仅在问题三、四使用 `storage_information.xlsx` 中相应字段；不猜测问题二边界。每小时复核功率、效率、SOC、购售电边界、分源守恒和终端 SOC。该规则是可行启发式；成本是否下降只由与无储能基准的当次复算差值判断，不预设改善，也不声明成本、碳、峰值或波动最优。
5. **问题四联合候选与情景**：任务侧使用问题四局部评分改进后的完整 incumbent；能源侧对该负荷应用问题三确定性储能规则。实际情景输入唯一规定为：电价情景 $price'=\bar p_r+\kappa(price-\bar p_r)$，$\kappa\in\{0.8,1.2\}$；新能源情景 $R'=\mu R$，$\mu\in\{0.8,1.2\}$，不加随机噪声。碳情景把 $0.9E_{base}$、$0.8E_{base}$ 作为硬目标，并实际生成响应决策：从同一完整 incumbent 出发，按 $GPU\_Demand\times Duration_h$ 取前2,000个弹性任务，依次使用固定碳惩罚阶梯 $(w_E,\kappa_C)\in\{(2,200),(4,500),(8,1000),(16,2000)\}$，其中任务无量纲 score 的 $z_E$ 权重改为 $w_E$，储能充放电排序量改为 $price+\kappa_C carbon$（CNY/MWh）。四个候选均复算完整碳排放，分别为10%和20%目标选择满足硬目标且成本最低者；无候选满足则输出 `target_infeasible_within_declared_heuristic`，不得伪造可行情景方案。电价、新能源情景也必须在变换后的逐时输入上重新运行能源规则并复算指标。固定输出顺序为 baseline、carbon10、price0.8、renewable0.8、carbon20、price1.2、renewable1.2。只有通过完整复算的行可标 `completed`；比例换算行一律无效。
6. **墙钟与固定阶段截止**：以同一 `time.monotonic()` 起点计时，累计截止固定为：输入与预测 45 s、q1 incumbent 70 s、q2 130 s、q3 160 s、q4 235 s、七情景 270 s、中间产物与应急 285 s、最终残差验证和文件写出 300 s。某阶段到达其截止仍未完成则整次 code 失败；不得挪用后续必答阶段预算，也不以部分结果冒充完成。

周期仅是 ACF 局部峰估计；服务质量始终标记为 `service_quality_proxy`，不是用户满意度。问题二统一新能源利用率为100%时只解释为无独立外送上限下的指标退化；问题三、四保守 SOC 子集只证明当前启发式可行，不能代表附件完整储能可行域的优化潜力。

实际结果可信等级最高为 `scenario-feasible`：所有正式数值必须来自当次结构化输出并通过硬约束复算；`global_optimality=not_certified`，滚动局部间隙、启发式评分和单点候选均不得解释为全时域全局最优或完整 Pareto 证书。为满足固定交付文件名，`q2_pareto.csv`、`q4_pareto.csv` 只保存“候选比较兼容表”，必须含 `ParetoCertified=false` 与 `Credibility=scenario-feasible`；正文和图题均称候选权衡图，不称已认证 Pareto 前沿。
