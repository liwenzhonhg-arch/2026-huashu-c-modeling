...
2. **任务调度**：完整 incumbent 的唯一构造顺序为“实时任务优先、LatestFinishHour 升序、EstimatedDuration_min 升序、TaskID 升序”；每个任务先尝试 ArrivalHour，在满足时延的区域中按网络时延、区域名顺序选择第一个容量可行位置；若弹性任务在到达时刻无位置，则按开工小时从 $ArrivalHour$ 到 $\lfloor LatestFinishHour-Duration_h\rfloor$ 升序、同小时按网络时延和区域名升序扫描完整允许域。只有完整允许域均无容量可行位置才停止并输出 `infeasible_incumbent`。实时任务仍只能在 ArrivalHour 开工。构造后复核全部 50,000 个任务，失败则整次停止。问题二、四的待改进集合只含非实时任务，按 $GPU\_Demand\times Duration_h$ 降序、LatestFinishHour 升序、TaskID 升序取前 10,000 个。每次先移除旧占用。对候选先计算设施增量能量 $E=\sum_t\mathrm{PUE}_rg_ip_k\omega_{it}(s)$，并由原始附件固定尺度 $\widetilde p=\operatorname{median}(price)>0$、$\widetilde c=\operatorname{median}(carbon)>0$。定义五个无量纲分量：$z_C=\sum_t E_tprice_{rt}/\max(1,E\widetilde p)$，$z_E=\sum_tE_tcarbon_{rt}/\max(1,E\widetilde c)$，$z_R=\sum_t\min(E_t,R_{rt}^{av})/\max(1,E)$，$z_W=(s-a_i)/\max(1,\lfloor f_i^{late}-d_i\rfloor-a_i)$，$z_L=\ell_{o_ir}/\max(1,\ell_i^{max})$。问题二评分固定为 $score=z_C+z_E-0.5z_R+0.5z_W+0.5z_L$；问题四固定为 $score=z_C+1.5z_E-z_R+z_W+0.5z_L$。按 score、开工小时、区域名升序选择；只有新 score 严格改善且全部任务硬约束仍满足才接受，否则恢复原位置。未搜索任务保持 incumbent 决策。该 score 无量纲，只是固定启发式排序量。
3. **问题二能源流**：无储能，逐时按新能源优先直供，余量外送，不足部分购电；严格复算成本、购电碳排放、时延、直接消纳率、外送率和退化的统一新能源利用率。只输出一个经验证的启发式候选，不把单点文件称为完整 Pareto 前沿。
