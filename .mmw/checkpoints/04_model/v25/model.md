...
前述各问 MILP 与 ε-约束体系是数学 formulation，用于定义目标、物理守恒和可行性；受 50,000 个任务与单次 300 s 共享墙钟限制，代码阶段不把完整滚动 MILP 当作已经执行的方法。实际 implementation 固定如下，二者必须在论文中分开叙述。
...
2. **任务调度**：先构造并复核一个覆盖全部 50,000 个任务的全时域可行 incumbent。问题二、四分别按 GPU-hour 影响从大到小，对最多 10,000 个弹性任务做确定性局部移位；每次先移除旧占用，仅接受同时满足到达、截止、时延、GPU、IT 和设施功率约束且当前标量评分改善的候选，否则恢复原位置。未搜索任务保持 incumbent 决策。
...
5. **问题四联合候选**：任务侧使用问题四局部评分改进后的完整 incumbent；能源侧对该负荷应用与问题三相同的储能可行规则。固定七情景的实际实现不生成随机噪声，只使用预声明比例元组 `(carbon_cut, price_scale, renewable_scale)`：baseline `(0,1,1)`、carbon10 `(0.1,1,1)`、price0.8 `(0,0.8,1)`、renewable0.8 `(0,1,0.8)`、carbon20 `(0.2,1,1)`、price1.2 `(0,1.2,1)`、renewable1.2 `(0,1,1.2)`，按此顺序生成。只有重新按情景输入运行完整任务和能源复算的行可标 `completed`；仅按指标比例换算的行必须标 `proxy_only` 或 `incomplete_discarded`，不把代理称为 Pareto 最优解。
6. **墙钟**：正式程序在 285 s 内完成读取、预测、两次有界局部改进、储能和指标复算；最后 15 s 预留给完整残差验证和文件写出。若任一必答部分未启动或未形成完整文件，整次 code 失败，不以部分结果冒充完成。
