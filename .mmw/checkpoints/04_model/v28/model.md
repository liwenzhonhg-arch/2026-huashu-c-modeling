...
5. **问题四联合候选与情景**：任务侧使用问题四局部评分改进后的完整 incumbent；能源侧对该负荷应用问题三确定性储能规则。实际情景输入唯一规定为：电价情景 $price'=\bar p_r+\kappa(price-\bar p_r)$，$\kappa\in\{0.8,1.2\}$；新能源情景 $R'=\mu R$，$\mu\in\{0.8,1.2\}$，不加随机噪声；碳情景只把 $0.9E_{base}$、$0.8E_{base}$ 作为硬目标。每个情景必须在变换后的逐时输入上重新运行能源规则并复算指标。若固定任务候选的复算碳排放不满足碳目标，则状态为 `target_not_met`，不得输出为可行情景方案。固定顺序为 baseline、carbon10、price0.8、renewable0.8、carbon20、price1.2、renewable1.2。只有通过完整复算的行可标 `completed`；比例换算行一律无效。
