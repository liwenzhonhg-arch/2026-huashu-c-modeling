...
2. **任务调度**：完整 incumbent 的唯一构造顺序为“实时任务优先、LatestFinishHour 升序、EstimatedDuration_min 升序、TaskID 升序”；每个任务固定在 ArrivalHour 开工，在满足时延的区域中按网络时延、区域名顺序选择第一个容量可行位置。构造后复核全部 50,000 个任务，失败则整次停止。问题二、四的待改进集合只含非实时任务，按 $GPU\_Demand\times Duration_h$ 降序、LatestFinishHour 升序、TaskID 升序取前 10,000 个。每次先移除旧占用；问题二/四候选评分分别固定为
$$
score=\sum_t[\mathrm{PUE}_r g_ip_k\omega_{it}(s)(price_{rt}+w_C carbon_{rt})-w_R\min(\mathrm{PUE}_r g_ip_k\omega_{it}(s),R_{rt}^{av})]+10(s-a_i)+\ell_{o_ir}g_id_i,
$$
其中问题二 $(w_C,w_R)=(200,10)$、问题四 $(300,20)$。按 score、开工小时、区域名升序选择；只有新 score 严格改善且全部任务硬约束仍满足才接受，否则恢复原位置。未搜索任务保持 incumbent 决策。该 score 只是固定启发式排序量，不解释为有物理单位的社会成本。
3. **问题二能源流**：无储能，逐时按新能源优先直供，余量外送，不足部分购电；严格复算成本、购电碳排放、时延、直接消纳率、外送率和退化的统一新能源利用率。只输出一个经验证的启发式候选，不把单点文件称为完整 Pareto 前沿。
4. **问题三储能**：物理可行域仍为 $MinSOC\le E_t\le StorageCapacity$；实际启发式额外采用保守限制 $E_t\ge InitialSOC$，这是可行域的子集而不是篡改附件下界。逐区域先计算真实购电价的 25%/75% 分位数 $p_{25},p_{75}$，从 `InitialSOC_MWh` 前向递推。每小时依次执行：$R^{use}=\min(P^{fac},R^{av})$，得到 `surplus/deficit`；新能源充电取 `min(surplus, MaxChargePower, (Capacity-E)/eta_c)`；仅当 $price\le p_{25}$ 且 $t<2406$ 时，在剩余充电功率、SOC 空间和购电上限内电网充电；仅当 $price\ge p_{75}$ 且 $t<2406$ 时，放电取 `min(deficit, MaxDischargePower, (E-InitialSOC)*eta_d)`；充电发生时放电固定为0，放电发生时充电固定为0；外送取新能源充电后的剩余新能源与 `SellLimit_MW/MaxGridExport_MW` 的最小值，余量弃电。第2406小时禁止放电且 $E_{2406}\ge InitialSOC$。缺失 `MaxGridImport/Export` 时仅在问题三、四使用 `storage_information.xlsx` 中相应字段；不猜测问题二边界。每小时复核功率、效率、SOC、购售电边界、分源守恒和终端 SOC。该规则是可行启发式，不声明成本、碳、峰值或波动最优。
