...
1. **预测**：24 h 与 168 h 季节基线均按固定预测起点递归计算；增强模型只评估四个预声明 Huber-Ridge 代表组合。按验证 WAPE、MAE、简单模型优先序唯一选择，再对 2376–2399 固定起点测试；收敛失败的增强候选丢弃并保留季节基线。
...
4. **问题三储能**：从 `InitialSOC_MWh` 出发，以价格四分位规则充放电；SOC 不得低于初始 SOC，低价/新能源富余时充电，高价时仅放出高于初始 SOC 的能量，第 2406 小时禁止放电。每小时复核充放电功率、效率、SOC、购售电边界、分源守恒和终端 SOC。该规则是可行启发式，不声明成本、碳、峰值或波动最优。
5. **问题四联合候选**：任务侧使用问题四局部评分改进后的完整 incumbent；能源侧对该负荷应用与问题三相同的储能可行规则。固定七情景只做同一候选的压力测试或在预算允许时重算，不把未重新优化的情景代理称为 Pareto 最优解。
6. **墙钟**：正式程序在 285 s 内完成读取、预测、两次有界局部改进、储能和指标复算；最后 15 s 预留给完整残差验证和文件写出。若任一必答部分未启动或未形成完整文件，整次 code 失败，不以部分结果冒充完成。

实际结果可信等级最高为 `scenario-feasible`：所有正式数值必须来自当次结构化输出并通过硬约束复算；`global_optimality=not_certified`，滚动局部间隙、启发式评分和单点候选均不得解释为全时域全局最优或完整 Pareto 证书。
