...
| $P_{rt}^{export,max},S_r^{max}$ | 区域外送边界、储能表外送上限 | 可观测参数 | MW |
...
重叠预览区不进入式（75）和式（78），防止等待量和峰值重复计数。最终方案必须按式（28）–（33）、（60）、（70）–（72）进行全时域复算。
...
仅当 $N_{launch}\ge1$ 且单候选的 $30$ s 上限能够放入总剩余时间时启动新候选。式（91）只是启动上限，不要求机械完成全部 $12$ 个组合。已启动候选可越过搜索截止，但不得越过总截止；否则中断并丢弃。最后 $15$ s 仅用于全时域复核和文件输出。
