...
def build_q4_task_candidate_set(
    ...
    基线任务方案沿用问题二结果；从单任务邻域中构造两个彼此不同且与基线
    不同的替代方案。每个替代方案均执行一次50000任务全量硬约束审计。
    15个情景复用任务审计结论，但分别重建设施负荷和储能策略。
    """
    ...
    order = list(
        baseline.sort_values(
            ["WaitHour", "GPU_Demand", "TaskID"],
            ascending=[False, False, True],
        ).head(max(Q2_TASK_POOL_SIZE, 8)).index
    )
    checked = 0
    alternatives = []
    ...
    for row_idx in order:
        if (
            len(alternatives) >= 2
            or checked >= max_checked
            or time.monotonic() >= SEARCH_LIMIT
        ):
            break
        old = baseline.loc[row_idx].copy()
        task = task_source.loc[old["TaskID"]]
        duration = float(old["FinishHour"] - old["StartHour"])
        arrival = int(task["ArrivalHour"])
        latest_start = int(np.floor(
            min(float(task["LatestFinishHour"]), 2406.0)
            - duration
            + TOL
        ))
        if task["TaskType"] == "RealTimeInference":
            ...
            starts = sorted(set([
                arrival,
                max(arrival, int(old["StartHour"]) - 1),
                min(latest_start, int(old["StartHour"]) + 1),
                latest_start,
            ]))
        ...
        for start in starts:
            if len(alternatives) >= 2:
                break
            for region in REGIONS:
                if checked >= max_checked or time.monotonic() >= SEARCH_LIMIT:
                    break
                if start < arrival or start > latest_start:
                    continue
                if (
                    region == old["ExecutionRegion"]
                    and start == int(old["StartHour"])
                ):
                    continue
                actual_latency = float(
                    latency[(task["SourceRegion"], region)]
                )
                ...
                checked += 1
                trial = baseline.copy()
                trial.at[row_idx, "ExecutionRegion"] = region
                trial.at[row_idx, "StartHour"] = start
                trial.at[row_idx, "FinishHour"] = start + duration
                trial.at[row_idx, "WaitHour"] = start - arrival
                trial.at[row_idx, "NetworkLatency_ms"] = actual_latency
                feasible, trial_used_gpu = schedule_candidate_feasible(
                    d, workload, trial
                )
                if feasible:
                    signature = tuple(
                        trial.loc[
                            trial["TaskID"].eq(old["TaskID"]),
                            ["TaskID", "ExecutionRegion", "StartHour"],
                        ].iloc[0].astype(str)
                    )
                    if all(
    ...
    if len(alternatives) < 2:
        raise RuntimeError(
            f"CON-Q4-2: actual={1 + len(alternatives)}, threshold=3, "
            "未构造出基线加两个相互不同的全量任务可行候选"
        )

    candidates = [
        {
            "candidate_id": "task_baseline",
            "schedule": baseline,
            "facility_override": baseline_flow[
                ["Region", "Hour", "FacilityLoad_MW"]
            ].copy(),
            "task_search_status": "q2_best_found_reused",
    ...
    for index, item in enumerate(alternatives, start=1):
        candidate_id = f"task_alternative_{index}"
        alternative = item["schedule"]
        alternative_audit = build_constraint_audit(
            d, workload, alternative, item["used_gpu"]
        )
        alternative_residual = float(np.max(np.maximum(
            alternative_audit.drop(
                columns=["Region", "constraint_satisfied"]
            ).to_numpy(float),
            0.0,
        )))
        if (
            alternative_residual > TOL
            or not alternative_audit["constraint_satisfied"].all()
        ):
            fail(
                "CON-Q4-1",
                alternative_residual,
                TOL,
                f"{candidate_id}未通过50000任务全量硬约束审计",
            )
        alternative_flow, _, _, _ = no_storage_energy(d, alternative)
        candidates.append({
            "candidate_id": candidate_id,
            "schedule": alternative,
            "facility_override": alternative_flow[
                ["Region", "Hour", "FacilityLoad_MW"]
            ].copy(),
            "task_search_status": "full_horizon_feasible_alternative",
            "full_task_audit_passed": True,
        })
        alternative_audits[candidate_id] = alternative_audit
        alternative_residuals[candidate_id] = alternative_residual

    evidence = {
        "status": "completed",
        "neighbor_candidates_checked": int(checked),
        "candidate_count": int(len(candidates)),
        "different_candidate_count": 2,
        "full_task_audits": 2,
        "alternative_max_constraint_residual": max(
            alternative_residuals.values()
        ),
        "alternative_constraint_residuals": alternative_residuals,
    }
    return candidates, evidence, alternative_audits
...
def run_q4_scenarios(d, task_candidates, joint_baseline):
    baseline_schedule = task_candidates[0]["schedule"]
    ...
    baseline_latency, baseline_service, baseline_migrated = service_metrics(
        baseline_schedule
    )
    rows = []
    trace_rows = []
    ...
    for scenario_type, parameter, level in scenario_specs:
        if time.monotonic() >= SEARCH_LIMIT:
            raise RuntimeError(
                f"CON-Q4-6: actual={time.monotonic() - PROGRAM_START:.6f}, "
                "threshold=270, 15个规定情景尚未全部完成"
            )

        evaluated = []
        for candidate in task_candidates:
            if (
                checked_total >= Q4_MOVE_LIMIT
                or time.monotonic() >= SEARCH_LIMIT
            ):
                break
            flow = scenario_storage_flow(
                d,
                candidate,
                scenario_type,
                level,
                volatile,
                baseline_flow=joint_baseline,
            )
            audit_storage_policy(d, flow, "CON-Q4-1")
            cost, carbon, renewable, peak, ramp = metrics_energy(flow)
            latency, service, migrated = service_metrics(
                candidate["schedule"]
            )
            checked_total += 1
            evaluated.append({
                "candidate_id": candidate["candidate_id"],
                "task_search_status": candidate["task_search_status"],
        ...
        if len(evaluated) != 3:
            raise RuntimeError(
                f"CON-Q4-5: actual={len(evaluated)}, threshold=3, "
                f"情景{scenario_type}:{level}未完成三个任务储能组合评价"
            )

        candidate_frame = pd.DataFrame(evaluated)
        objective_columns = [
            "cost_CNY",
            "carbon_tCO2",
            "network_latency_ms",
            "peak_grid_purchase_MW",
            "ramp_MW",
        ]
        objective_values = candidate_frame[objective_columns].to_numpy(float)
        scale = np.maximum(np.abs(objective_values[0]), 1.0)
        scores = np.mean(
            (objective_values - objective_values[0]) / scale,
            axis=1,
        )
        scores += (
            candidate_frame["service_quality"].iloc[0]
            - candidate_frame["service_quality"].to_numpy(float)
        )
        scores += (
            candidate_frame["renewable_utilization"].iloc[0]
            - candidate_frame["renewable_utilization"].to_numpy(float)
        )
        candidate_frame["score"] = scores

        round_one = candidate_frame[
            candidate_frame["candidate_id"].isin(
                ["task_baseline", "task_alternative_1"]
            )
        ]
        round_one_index = round_one["score"].idxmin()
        incumbent_index = round_one_index
        round_one_selected = str(
            candidate_frame.loc[round_one_index, "candidate_id"]
        )
        trace_rows.append({
            ...
            "candidate_scores": json.dumps(
                {
                    str(row.candidate_id): finite(row.score, "q4_score")
                    for row in round_one.itertuples()
                },
                ensure_ascii=False,
            ),
            "accepted": bool(round_one_selected != "task_baseline"),
            "incumbent": round_one_selected,
        })
        alternative_two_index = candidate_frame.index[
            candidate_frame["candidate_id"].eq("task_alternative_2")
        ][0]
        accepted_round_two = bool(
            candidate_frame.loc[alternative_two_index, "score"]
            < candidate_frame.loc[incumbent_index, "score"] - 1e-12
        )
        if accepted_round_two:
            incumbent_index = alternative_two_index
        trace_rows.append({
            "scenario_type": scenario_type,
            "parameter": parameter,
            "level": level,
            "alternation_round": 2,
            "evaluated_candidates": json.dumps(
                ["task_alternative_2"], ensure_ascii=False
            ),
            "candidate_scores": json.dumps({
                "task_alternative_2": finite(
                    candidate_frame.loc[alternative_two_index, "score"],
                    "q4_score",
                )
            }, ensure_ascii=False),
            "accepted": accepted_round_two,
            "incumbent": str(
                candidate_frame.loc[incumbent_index, "candidate_id"]
            ),
        })
        ...
        if scenario_type == "carbon_constraint":
            ...
            eligible = candidate_frame["carbon_tCO2"] <= target + TOL
            if eligible.any():
                selected_index = candidate_frame.loc[
                    eligible, "score"
                ].idxmin()
            else:
                selected_index = candidate_frame["score"].idxmin()
                status = "target_infeasible_within_declared_heuristic"
        else:
            selected_index = candidate_frame["score"].idxmin()

        selected = candidate_frame.loc[selected_index]
        scenario_trace = [
        ...
        if selected["candidate_id"] == "task_baseline":
            task_status = "searched_no_improvement"
        else:
            task_status = "alternative_selected"
        rows.append({
            "scenario_type": scenario_type,
            "parameter": parameter,
            "level": level,
            "status": status,
            "alternation_rounds": 2,
            "trace": json.dumps(scenario_trace, ensure_ascii=False),
            "checked_candidates": len(candidate_frame),
            "selected_candidate": selected["candidate_id"],
            "migration_count": int(selected["migration_count"]),
            "task_search_status": task_status,
            "cost_CNY": selected["cost_CNY"],
            "carbon_tCO2": selected["carbon_tCO2"],
            "network_latency_ms": selected["network_latency_ms"],
            "service_quality": selected["service_quality"],
            "renewable_utilization": selected["renewable_utilization"],
            "peak_grid_purchase_MW": selected["peak_grid_purchase_MW"],
...
def schedule_candidate_feasible(d, workload, schedule):
    ...
    rt = d["region"].set_index(["Region", "Hour"])
    for row in schedule.itertuples():
        task = task_source.loc[row.TaskID]
        arrival = float(task["ArrivalHour"])
        if float(row.StartHour) < arrival - TOL:
            return False, None
        if task["TaskType"] == "RealTimeInference":
            if abs(float(row.StartHour) - arrival) > TOL:
                return False, None
        finish_limit = min(float(task["LatestFinishHour"]), 2406.0)
        if float(row.FinishHour) > finish_limit + TOL:
            return False, None
        actual_latency = float(
            latency[(task["SourceRegion"], row.ExecutionRegion)]
        )
        if actual_latency > float(task["MaxLatency_ms"]) + TOL:
            return False, None
    for idx, region in enumerate(REGIONS):
        nonai = np.array([
            float(rt.loc[(region, hour), "NonAI_IT_Load_MW"])
            for hour in range(2407)
        ])
        if np.max(
            used_gpu[idx] - float(gpu.loc[region, "Available_GPU"])
        ) > TOL:
            return False, None
        it_load = nonai + used_power[idx]
        if np.max(
...
def q2_local_search(d, workload, baseline_schedule, max_candidates=Q2_MOVE_LIMIT):
    current = baseline_schedule.copy().reset_index(drop=True)
    flow, cost, carbon, renewable = no_storage_energy(d, current)
    baseline = q2_search_metrics(current, cost, carbon)
    ...
    # 原代码会对全部50000个任务建立候选，并对每个候选复制和复核完整调度表。
    # 有限搜索优先检查等待时间长、GPU需求大的任务；候选本身仍执行完整任务、
    # 容量和能源复算，因此不是抽样指标或占位结果。
    order = list(current.sort_values(
        ["WaitHour", "GPU_Demand", "TaskID"],
        ascending=[False, False, True],
    ).head(Q2_TASK_POOL_SIZE).index)

    for row_idx in order:
        if checked >= max_candidates or time.monotonic() >= SEARCH_LIMIT:
            break
        old = current.loc[row_idx].copy()
        task = task_source.loc[old["TaskID"]]
        duration = float(old["FinishHour"] - old["StartHour"])
        arrival = int(task["ArrivalHour"])
        latest_start = int(np.floor(
            min(float(task["LatestFinishHour"]), 2406.0) - duration + TOL
        ))
        if task["TaskType"] == "RealTimeInference":
            starts = [arrival]
        else:
        ...
        for start in starts:
            for region in REGIONS:
                ...
                feasible, _ = schedule_candidate_feasible(d, workload, trial)
                if not feasible:
                    continue
                try:
                    trial_flow, trial_cost, trial_carbon, trial_re = (
                        no_storage_energy(d, trial)
                    )
                except RuntimeError:
                    continue
                metrics = q2_search_metrics(
                    trial, trial_cost, trial_carbon
                )
                scale = np.maximum(np.abs(baseline), 1.0)
                score = float(np.mean((metrics - incumbent) / scale))
                if score < -1e-10:
                    # 只接受经过完整复算的严格改善可行候选。
                    current = trial
                    flow = trial_flow
    ...
    summary = {
        ...
        "task_pool_size": int(len(order)),
        "status": "improved" if accepted else "searched_no_improvement",
        "checked_candidates": checked,
        "accepted_improvements": accepted,
        "baseline_cost_CNY": baseline[0],
        "final_cost_CNY": incumbent[0],
        "baseline_carbon_tCO2": baseline[1],
        "final_carbon_tCO2": incumbent[1],
        "baseline_latency_ms": baseline[2],
        "final_latency_ms": incumbent[2],
        "baseline_wait_h": baseline[3],
        "final_wait_h": incumbent[3],
    }
    return current, flow, cost, carbon, renewable, summary, pd.DataFrame(trace)
...
def storage_local_search(d, facility_override=None, max_candidates=12000):
    ...
    baseline = build_storage_policy(
        d, facility_override=facility_override
    )
    audit_storage_policy(d, baseline, "CON-Q3-1")
    incumbent = baseline
    incumbent_metrics = np.asarray(metrics_energy(incumbent), dtype=float)
    first_metrics = incumbent_metrics.copy()
    checked = 1
    accepted = 0
    trace = []
    for low_q, high_q in policies:
        if checked >= max_candidates or time.monotonic() >= SEARCH_LIMIT:
            break
        checked += 1
        trial = build_storage_policy(
            d,
            facility_override=facility_override,
            charge_quantile=low_q,
            discharge_quantile=high_q,
        )
        audit_storage_policy(d, trial, "CON-Q3-1")
        metrics = np.asarray(metrics_energy(trial), dtype=float)
        selected = [0, 1, 3, 4]
        scale = np.maximum(np.abs(first_metrics[selected]), 1.0)
        score = float(np.mean(
            (metrics[selected] - incumbent_metrics[selected]) / scale
        ))
        if score < -1e-10:
            incumbent = trial
    ...
    summary = {
        ...
        "checked_candidates": checked,
        "accepted_improvements": accepted,
        "baseline_cost_CNY": first_metrics[0],
        "final_cost_CNY": incumbent_metrics[0],
        "baseline_carbon_tCO2": first_metrics[1],
        "final_carbon_tCO2": incumbent_metrics[1],
        "baseline_peak_MW": first_metrics[3],
        "final_peak_MW": incumbent_metrics[3],
        "baseline_ramp_MW": first_metrics[4],
        "final_ramp_MW": incumbent_metrics[4],
        "charge_MWh": float(incumbent["Charge_MW"].sum()),
        "discharge_MWh": float(incumbent["Discharge_MW"].sum()),
    }
    return incumbent, summary, pd.DataFrame(trace)
...
def _optimize_q2_scheme(d, workload, baseline_schedule, scheme, budget, offset):
    ...
    priority = schedule.copy()
    priority["_GPUh"] = priority["GPU_Demand"] * (
        priority["FinishHour"] - priority["StartHour"]
    )
    ordered = list(priority.sort_values(
        ["_GPUh", "WaitHour", "TaskID"], ascending=[False, False, True]
    ).index)
    if ordered:
    ...
    for row_idx in ordered:
        ...
        if task["TaskType"] == "RealTimeInference":
            ...
            starts = sorted(set([
                arrival, int(old["StartHour"]),
                max(arrival, int(old["StartHour"]) - 6),
                max(arrival, int(old["StartHour"]) - 1),
                min(latest_start, int(old["StartHour"]) + 1),
                min(latest_start, int(old["StartHour"]) + 6),
                latest_start,
            ]))
        ...
        for start in starts:
            ...
            for region in REGIONS:
                ...
                if region == old["ExecutionRegion"] and start == int(old["StartHour"]):
                    continue
                actual_latency = float(latency[(task["SourceRegion"], region)])
                if actual_latency > float(task["MaxLatency_ms"]) + TOL:
                    continue
                checked += 1
                ...
                deltas = _move_deltas(
                    old["ExecutionRegion"], float(old["StartHour"]),
                    float(old["FinishHour"]), region, float(start), finish,
                    float(old["GPU_Demand"]), task_power, context,
                )
                weight = float(old["GPU_Demand"]) * duration
                service_delta = weight * (
                    actual_latency - float(old["NetworkLatency_ms"])
                    + 10.0 * (start - float(old["StartHour"]))
                )
                delta, reason = _evaluate_incremental_move(
                    context, deltas, service_delta
                )
                accepted_move = False
                score = None
                ...
                trace.append({
                    "scheme": scheme,
                    "candidate_no": checked,
                    "TaskID": int(task.name),
                    "from_region": str(old["ExecutionRegion"]) if not accepted_move else "updated",
                    "to_region": region,
                    "start_hour": start,
                    "status": reason,
                    "score": score,
    ...
    summary = {
        "scheme": scheme,
        "declared_candidate_limit": int(budget),
        "checked_candidates": int(checked),
        "unique_tasks_visited": int(len(visited)),
...
def q2_local_search(d, workload, baseline_schedule, max_candidates=Q2_MOVE_LIMIT):
    ...
    baseline_flow, baseline_cost, baseline_carbon, baseline_re = no_storage_energy(
        d, baseline_schedule
    )
    empty_flow, nonai_cost, nonai_carbon, _ = no_storage_energy(
        d, baseline_schedule.iloc[0:0].copy()
    )
    schemes = [
    ...
    for index, scheme in enumerate(schemes):
        schedule, flow, summary, trace, audit = _optimize_q2_scheme(
            d, workload, baseline_schedule, scheme, per_scheme, index * 997
        )
        summary.update({
            "baseline_cost_CNY": baseline_cost,
            "baseline_carbon_tCO2": baseline_carbon,
            "baseline_renewable_utilization": baseline_re,
            "nonAI_cost_CNY": nonai_cost,
        ...
        Q2_SCHEME_RESULTS[scheme] = {
            "schedule": schedule, "flow": flow, "summary": summary,
        }
        Q2_SCHEME_AUDITS[scheme] = audit
        all_trace.append(trace)
    selected = Q2_SCHEME_RESULTS["balanced"]
    summary = dict(selected["summary"])
    all_trace_frame = pd.concat(all_trace, ignore_index=True)
    unique_tasks = int(len(set(
        all_trace_frame["TaskID"].dropna().astype(int)
    )))
    summary.update({
        "declared_candidate_limit": int(max_candidates),
        "checked_candidates": int(sum(
            value["summary"]["checked_candidates"]
            for value in Q2_SCHEME_RESULTS.values()
        )),
        "unique_tasks_visited": unique_tasks,
        "task_coverage_rate": float(unique_tasks / len(workload)),
        "all_schemes_accepted_improvements": int(sum(
            value["summary"]["accepted_improvements"]
            for value in Q2_SCHEME_RESULTS.values()
        )),
        "status": "improved" if summary["accepted_improvements"] else "searched_no_improvement",
        "task_pool_size": len(workload),
        "baseline_latency_ms": service_metrics(baseline_schedule)[0],
        ...
        "final_cost_CNY": summary["cost_CNY"],
        "final_carbon_tCO2": summary["carbon_tCO2"],
    })
    return (
        selected["schedule"], selected["flow"], summary["cost_CNY"],
        summary["carbon_tCO2"], summary["renewable_utilization"], summary,
        all_trace_frame,
    )
...
def storage_local_search(d, facility_override=None, max_candidates=12000):
    ...
    policy_names = [
        "no_action", "cost_min", "carbon_min", "renewable_max",
        "peak_min", "ramp_min", "balanced",
    ]
    flows = {"no_action": _no_action_policy(d, facility_override=facility_override)}
    for name in policy_names[1:]:
        flows[name] = _storage_dp_policy(
            d, name, facility_override=facility_override
        )
    rows = []
    seen_hashes = {}
    for name in policy_names:
        ...
        equivalent_cycles = float(sum(
            (
                flow.loc[flow["Region"].eq(region), "Charge_MW"].sum()
                * float(storage.loc[region, "ChargeEfficiency"])
                + flow.loc[flow["Region"].eq(region), "Discharge_MW"].sum()
                / float(storage.loc[region, "DischargeEfficiency"])
    ...
    balanced_row = next(row for row in rows if row["policy"] == "balanced")
    if balanced_row["nondominated"]:
        selected_name = "balanced"
    else:
        indices = [i for i, flag in enumerate(nondominated) if flag]
        matrix = np.asarray([
            [rows[i]["cost_CNY"], rows[i]["carbon_tCO2"],
             -rows[i]["renewable_utilization"], rows[i]["peak_MW"], rows[i]["ramp_MW"]]
            for i in indices
        ], dtype=float)
        minimum = matrix.min(axis=0)
        span = np.maximum(matrix.max(axis=0) - minimum, 1.0)
        selected_name = rows[indices[int(np.argmin(
            np.linalg.norm((matrix - minimum) / span, axis=1)
        ))]]["policy"]
    ...
    summary = {
        ...
        "baseline_cost_CNY": base["cost_CNY"],
        "final_cost_CNY": final["cost_CNY"],
        "baseline_carbon_tCO2": base["carbon_tCO2"],
        "final_carbon_tCO2": final["carbon_tCO2"],
        "baseline_peak_MW": base["peak_MW"],
        "final_peak_MW": final["peak_MW"],
        "baseline_ramp_MW": base["ramp_MW"],
        "final_ramp_MW": final["ramp_MW"],
        "charge_MWh": final["charge_MWh"],
        "discharge_MWh": final["discharge_MWh"],
    ...
    if facility_override is None:
        relaxed = _storage_dp_policy(
            d, "cost_min", facility_override=None, terminal_mode="free"
        )
        Q3_TERMINAL_SENSITIVITY = {
            "initial_equal": next(row for row in rows if row["policy"] == "cost_min"),
            "terminal_free": {
                "metrics": metrics_energy(relaxed),
                "terminal_soc": relaxed.groupby("Region").tail(1)[
                    ["Region", "SOC_MWh"]
                ].to_dict("records"),
            },
        }
...
def main():
    ...
    q4_scenarios.to_csv(
        RESULT_DIR / "q4_scenario_comparison_v42.csv", index=False
    )
