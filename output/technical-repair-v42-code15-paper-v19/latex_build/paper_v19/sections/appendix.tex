...
图表也采用相同边界。每幅数值图必须能追溯到 solve v12 当前 CSV；旧 paper 构建目录中遗留但未出现在当前图形清单的图片不进入 v19。预测比较图由当前预测明细按小时聚合，Q1 GPU、Q3 SOC 及两幅灵敏度图直接来自 solve v12 正式图形。论文中的亿元、万元和百分比只是展示换算，审计仍以原始元、MWh、MW 与无量纲值为准。
...
方法的正式实现为 code v15 的 \texttt{solution.py}。本次 v19 只重建论文与审核证据，没有修改该程序、solve v12 结构化结果或既有 v17/v18 文件。
